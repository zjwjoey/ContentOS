...
export interface SegmentationSettingsV1 { mode: SegmentationModeV1; delimiters?: string[] }
...
export const DEFAULT_SUBTITLE_STYLE_V1: SubtitleStyleV1 = {
  schemaVersion: 'EDIT_PRESENTATION_V1', fontId: 'sans-serif', fontSize: 48, color: '#FFFFFF',
  outline: { enabled: true, color: '#000000', width: 2 }, shadow: { enabled: false, color: '#000000', x: 2, y: 2, blur: 0 },
  background: { enabled: false, color: '#000000', opacity: 0.45, padding: 12 }, position: { x: 0.5, y: 0.86 }, align: 'CENTER',
  maxWidth: 0.88, maxLines: 2, lineHeight: 1.2, animation: 'NONE',
};
export const DEFAULT_PRESENTATION_SETTINGS_V1: PresentationSettingsV1 = { schemaVersion: 'EDIT_PRESENTATION_V1', canvas: DEFAULT_CANVAS_SETTINGS_V1, subtitleStyle: DEFAULT_SUBTITLE_STYLE_V1, segmentation: { mode: 'COMMA_SENTENCE' } };
...
export function normalizePresentationSettings(value?: Partial<PresentationSettingsV1>): PresentationSettingsV1 {
  ...
  return { schemaVersion: 'EDIT_PRESENTATION_V1', canvas, subtitleStyle: style, segmentation: { mode: value?.segmentation?.mode || 'COMMA_SENTENCE', ...(value?.segmentation?.delimiters ? { delimiters: value.segmentation.delimiters } : {}) } };
}
...
export function validatePresentationSettings(value: PresentationSettingsV1): void {
  ...
  if (!Number.isFinite(value.subtitleStyle.fontSize) || value.subtitleStyle.fontSize <= 0 || value.subtitleStyle.maxLines < 1 || value.subtitleStyle.maxLines > 3) throw new Error('Subtitle style is invalid');
}
