export type ApprovalTargetType = 'SCRIPT' | 'STORYBOARD' | 'RENDER' | 'PUBLISH';
...
export function validateApprovalDecision(decision: ApprovalDecisionV0): void {
  ...
  if (!['SCRIPT', 'STORYBOARD', 'RENDER', 'PUBLISH'].includes(decision.targetType)) throw new Error('targetType is invalid');
