...
export function createRuntimeDigitalHumanProviders(env: RuntimeDigitalHumanEnvironment = process.env as RuntimeDigitalHumanEnvironment): RuntimeDigitalHumanProviders {
  ...
  const avatar = avatarId === 'fake-avatar'
    ...
    : hzagentApiKey && (avatarId !== 'hzagent' || hzagentMappingConfigured)
      ? avatarId === 'hzagent'
        ? new HzAgentAvatarProvider({ baseUrl: env.HZAGENT_BASE_URL || 'https://api.ai.hzagent.cn', apiKey: hzagentApiKey, capabilitiesPath: env.HZAGENT_CAPABILITIES_PATH!, submitPath: env.HZAGENT_SUBMIT_PATH!, taskPath: env.HZAGENT_TASK_PATH!, authHeaderName: env.HZAGENT_AUTH_HEADER!, authScheme: env.HZAGENT_AUTH_SCHEME!, ...(requestTimeoutMs === undefined ? {} : { requestTimeoutMs }), ...(capabilityTimeoutMs === undefined ? {} : { capabilityTimeoutMs }) })
        : new HttpAvatarProvider({ providerId: avatarId, baseUrl: env.HZAGENT_BASE_URL || 'https://api.ai.hzagent.cn', apiKey: hzagentApiKey, ...(env.HZAGENT_CAPABILITIES_PATH ? { capabilitiesPath: env.HZAGENT_CAPABILITIES_PATH } : {}), ...(env.HZAGENT_SUBMIT_PATH ? { submitPath: env.HZAGENT_SUBMIT_PATH } : {}), ...(env.HZAGENT_TASK_PATH ? { taskPath: env.HZAGENT_TASK_PATH } : {}), ...(env.HZAGENT_AUTH_HEADER ? { authHeaderName: env.HZAGENT_AUTH_HEADER } : {}), ...(env.HZAGENT_AUTH_SCHEME !== undefined ? { authScheme: env.HZAGENT_AUTH_SCHEME } : {}), ...(requestTimeoutMs === undefined ? {} : { requestTimeoutMs }), ...(capabilityTimeoutMs === undefined ? {} : { capabilityTimeoutMs }) })
      : new UnavailableAvatarProvider(avatarId, !hzagentApiKey ? 'Avatar provider API key is not configured' : 'HZAgent endpoint and authentication mapping is not configured');
