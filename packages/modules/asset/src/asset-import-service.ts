...
export class AssetImportService {
  ...
  async createStaged(input: CreateStagedAssetImportInput): Promise<AssetImportRecord> {
    if (!input.projectId.trim() || !['VIDEO', 'AUDIO'].includes(input.kind) || !Number.isSafeInteger(input.byteSize) || input.byteSize <= 0) throw new Error('Asset import input is invalid');
    const result = await this.db.query('insert into asset_imports (id, project_id, original_name, kind, byte_size, staged_path, state, correlation_id) values ($1, $2, $3, $4, $5, $6, $7, $8) returning *', [`asset-import-${randomUUID()}`, input.projectId, safeName(input.originalName), input.kind, input.byteSize, safeStagePath(input.stagedPath), 'STAGED', input.correlationId]);
