...
export interface LocalMediaAsset {
  ...
  shots?: Array<{ index: number; sourceInMs: number; sourceOutMs: number; confidence: number; snapshotId: string }>;
  available: boolean;
  errorMessage?: string;
...
export class LocalMediaSourceService {
  ...
  async getScan(scanId: string, projectId?: string, workspaceId?: string): Promise<{ id: string; projectId: string; workspaceId: string; sourceRootId: string; sourceRoot: string; recursive: boolean; status: string; progress: Record<string, unknown>; error: unknown; files: LocalMediaAsset[] } | null> {
    if (!this.db) throw new Error('LOCAL_MEDIA_DATABASE_REQUIRED');
    const row = (await this.db.query('select * from local_media_scans where id = $1 and (($2::text is null and $3::text is null) or project_id = $2 or workspace_id = $3)', [scanId, projectId || null, workspaceId || null])).rows[0] as Record<string, unknown> | undefined;
    if (!row) return null;
    const files = await this.db.query('select f.*, i.tags as index_tags, i.usage_count, i.last_used_at, i.thumbnail_key as index_thumbnail_key, i.thumbnail_status as index_thumbnail_status, i.thumbnail_error as index_thumbnail_error, i.category as index_category from local_media_scan_files f left join local_media_index i on i.file_id = f.file_id where f.scan_id = $1 order by f.relative_path', [scanId]);
    return { id: String(row.id), projectId: row.project_id ? String(row.project_id) : '', workspaceId: row.workspace_id ? String(row.workspace_id) : '', sourceRootId: String(row.source_root_id), sourceRoot: String(row.source_root), recursive: Boolean(row.recursive), status: String(row.status), progress: (row.progress || {}) as Record<string, unknown>, error: row.error, files: files.rows.map((file) => ({ fileName: String(file.file_name), relativePath: String(file.relative_path), sourcePath: String(file.source_path), durationMs: Number(file.duration_ms), width: Number(file.width), height: Number(file.height), ...(file.fps != null ? { fps: Number(file.fps) } : {}), format: String(file.format), ...(file.codec ? { codec: String(file.codec) } : {}), orientation: (file.orientation || 'UNKNOWN') as LocalMediaAsset['orientation'], ...(file.file_size != null ? { fileSize: Number(file.file_size) } : {}), ...(file.modified_at ? { modifiedAt: new Date(String(file.modified_at)).toISOString() } : {}), tags: Array.isArray(file.index_tags) ? file.index_tags as string[] : Array.isArray(file.tags) ? file.tags as string[] : [], usageCount: Number(file.usage_count || 0), ...(file.last_used_at ? { lastUsedAt: new Date(String(file.last_used_at)).toISOString() } : {}), ...(file.index_category ? { category: String(file.index_category) } : {}), ...(file.index_thumbnail_key ? { thumbnailKey: String(file.index_thumbnail_key) } : {}), ...(file.index_thumbnail_status ? { thumbnailStatus: file.index_thumbnail_status as LocalMediaAsset['thumbnailStatus'] } : file.thumbnail_status ? { thumbnailStatus: file.thumbnail_status as LocalMediaAsset['thumbnailStatus'] } : {}), ...(file.index_thumbnail_error ? { thumbnailError: String(file.index_thumbnail_error) } : {}), available: Boolean(file.available), ...(file.error_message ? { errorMessage: String(file.error_message) } : {}) }) as LocalMediaAsset) };
  }
  ...
  async getLatestWorkspaceScan(workspaceId: string, sourceRootId: string): Promise<Awaited<ReturnType<LocalMediaSourceService['getScan']>>> { if (!this.db) throw new Error('LOCAL_MEDIA_DATABASE_REQUIRED'); const row = (await this.db.query("select id from local_media_scans where workspace_id = $1 and source_root_id = $2 and status = 'SUCCEEDED' order by scanned_at desc nulls last, created_at desc limit 1", [workspaceId, sourceRootId])).rows[0] as { id?: string } | undefined; return row?.id ? this.getScan(String(row.id), undefined, workspaceId) : null; }

  async getFile(sourceRootId: string, fileId: string, projectId?: string, workspaceId?: string): Promise<LocalMediaAsset | null> {
    if (!this.db) throw new Error('LOCAL_MEDIA_DATABASE_REQUIRED');
    const row = (await this.db.query("select f.* from local_media_scan_files f join local_media_scans s on s.id = f.scan_id where s.source_root_id = $1 and f.file_id = $2 and s.status = 'SUCCEEDED' and (($3::text is null and $4::text is null) or s.project_id = $3 or s.workspace_id = $4) order by s.scanned_at desc nulls last limit 1", [sourceRootId, fileId, projectId || null, workspaceId || null])).rows[0] as Record<string, unknown> | undefined;
    if (!row) return null;
    return ({ fileName: String(row.file_name), relativePath: String(row.relative_path), sourcePath: String(row.source_path), durationMs: Number(row.duration_ms), width: Number(row.width), height: Number(row.height), ...(row.fps != null ? { fps: Number(row.fps) } : {}), format: String(row.format), ...(row.codec ? { codec: String(row.codec) } : {}), orientation: (row.orientation || 'UNKNOWN') as LocalMediaAsset['orientation'], ...(row.file_size != null ? { fileSize: Number(row.file_size) } : {}), ...(row.modified_at ? { modifiedAt: new Date(String(row.modified_at)).toISOString() } : {}), tags: Array.isArray(row.tags) ? row.tags as string[] : [], ...(row.category ? { category: String(row.category) } : {}), ...(row.thumbnail_key ? { thumbnailKey: String(row.thumbnail_key) } : {}), ...(row.thumbnail_status ? { thumbnailStatus: row.thumbnail_status as LocalMediaAsset['thumbnailStatus'] } : {}), available: Boolean(row.available), ...(row.error_message ? { errorMessage: String(row.error_message) } : {}) } as LocalMediaAsset);
  }
  ...
  async listWorkspaceIndexPage(workspaceId: string, filters: { query?: string; orientation?: LocalMediaAsset['orientation'] | 'ALL'; category?: string; usage?: 'ALL' | 'UNUSED' | 'RECENT' | 'FREQUENT'; sort?: 'NAME' | 'UPDATED' | 'DURATION' | 'USAGE' | 'RECENT' | 'RECOMMENDED' | 'NEWEST' | 'LEAST_USED' | 'MOST_RECENT'; page?: number; pageSize?: number } = {}): Promise<{ items: LocalMediaAsset[]; total: number; page: number; pageSize: number }> {
    ...
    const fileIds = result.rows.map((row) => String(row.file_id));
    const shotRows = fileIds.length ? await this.db.query('select distinct on (r.asset_id,s.shot_index) r.asset_id,r.snapshot_id,s.shot_index,s.source_in_ms,s.source_out_ms,s.confidence from script_editing_v3_shot_detection_runs r join material_pool_snapshots p on p.id=r.snapshot_id left join script_editing_v3_shots s on s.run_id=r.id where p.workspace_id=$1 and r.asset_id = any($2::text[]) and r.status=\'SUCCEEDED\' order by r.asset_id,s.shot_index,r.created_at desc', [workspaceId, fileIds]) : { rows: [] };
    const shotsByAsset = new Map<string, Array<{ index: number; sourceInMs: number; sourceOutMs: number; confidence: number; snapshotId: string }>>();
    for (const row of shotRows.rows as Array<Record<string, unknown>>) { const assetId = String(row.asset_id); const shots = shotsByAsset.get(assetId) || []; if (row.shot_index !== null && row.shot_index !== undefined) shots.push({ index: Number(row.shot_index), sourceInMs: Number(row.source_in_ms), sourceOutMs: Number(row.source_out_ms), confidence: Number(row.confidence), snapshotId: String(row.snapshot_id) }); shotsByAsset.set(assetId, shots); }
    const items = result.rows.map((row) => ({ fileId: String(row.file_id), fileName: String(row.file_name), relativePath: String(row.relative_path), sourcePath: String(row.source_path), durationMs: Number(row.duration_ms), width: Number(row.width), height: Number(row.height), ...(row.fps != null ? { fps: Number(row.fps) } : {}), ...(row.codec ? { codec: String(row.codec) } : {}), ...(row.file_size != null ? { fileSize: Number(row.file_size) } : {}), ...(row.modified_at ? { modifiedAt: new Date(String(row.modified_at)).toISOString() } : {}), ...(row.source_fingerprint ? { sourceFingerprint: String(row.source_fingerprint) } : {}), tags: Array.isArray(row.tags) ? row.tags as string[] : [], ...(row.category ? { category: String(row.category) } : {}), usageCount: Number(row.usage_count || 0), candidateCount: Number(row.candidate_count || 0), selectedCount: Number(row.selected_count || 0), manualSelectCount: Number(row.manual_select_count || 0), finalUseCount: Number(row.final_use_count || 0), replaceCount: Number(row.replace_count || 0), recentUseCount: Number(row.recent_use_count || 0), jianyingUseCount: Number(row.jianying_use_count || 0), ...(row.usage_last_used_at ? { lastUsedAt: new Date(String(row.usage_last_used_at)).toISOString() } : row.last_used_at ? { lastUsedAt: new Date(String(row.last_used_at)).toISOString() } : {}), ...(row.thumbnail_key ? { thumbnailKey: String(row.thumbnail_key) } : {}), ...(row.thumbnail_status ? { thumbnailStatus: row.thumbnail_status as LocalMediaAsset['thumbnailStatus'] } : {}), available: row.availability === 'AVAILABLE', gold: Boolean(row.usage_gold ?? row.gold), disabled: Boolean(row.disabled), ...(shotsByAsset.has(String(row.file_id)) ? { shots: shotsByAsset.get(String(row.file_id)) } : {}) }) as LocalMediaAsset);
    return { items, total: Number((countResult.rows[0] as { total?: number } | undefined)?.total || 0), page, pageSize };
  }
  ...
  async updateWorkspaceTags(fileIds: string[], workspaceId: string, tags: string[]): Promise<number> { if (!this.db) throw new Error('LOCAL_MEDIA_DATABASE_REQUIRED'); if (!fileIds.length) return 0; const normalized = JSON.stringify([...new Set(tags.map((tag) => tag.trim()).filter(Boolean))]); const result = await this.db.query(`update local_media_index i set tags=$3,updated_at=now() where i.file_id=any($1::text[]) and exists (select 1 from local_media_scan_files f join local_media_scans s on s.id=f.scan_id where f.file_id=i.file_id and s.workspace_id=$2 and s.status='SUCCEEDED')`, [fileIds, workspaceId, normalized]); return result.rowCount || 0; }
  ...
  async updateDisabled(fileId: string, disabled: boolean): Promise<void> { if (!this.db) throw new Error('LOCAL_MEDIA_DATABASE_REQUIRED'); await this.db.query('update local_media_index set disabled=$2,updated_at=now() where file_id=$1', [fileId, disabled]); }
  ...
  async generateThumbnail(fileId: string, ffmpegPath = process.env.FFMPEG_PATH || 'ffmpeg'): Promise<{ key: string; path: string } | null> {
    if (!this.db) throw new Error('LOCAL_MEDIA_DATABASE_REQUIRED');
    const row = (await this.db.query<{ source_root_id: string; relative_path: string; source_path: string; duration_ms: number; modified_at: string | null; file_size: number | null }>('select i.source_root_id, i.relative_path, f.source_path, i.duration_ms, i.modified_at, i.file_size from local_media_index i join local_media_scan_files f on f.file_id = i.file_id join local_media_scans s on s.id = f.scan_id and s.status = \'SUCCEEDED\' where i.file_id = $1 and i.availability = \'AVAILABLE\' and f.available = true order by s.scanned_at desc nulls last, s.created_at desc limit 1', [fileId])).rows[0];
    if (!row) return null;
    ...
    try { await accessFile(path); await this.db.query('update local_media_index set thumbnail_key = $2, thumbnail_status = \'READY\', thumbnail_error = null, updated_at = now() where file_id = $1', [fileId, key]); return { key, path }; } catch { /* regenerate below */ }
    await this.db.query('update local_media_index set thumbnail_key = $2, thumbnail_status = \'PENDING\', thumbnail_error = null, updated_at = now() where file_id = $1', [fileId, key]);
    try { await generateVideoThumbnail(row.source_path, path, ffmpegPath, Number(row.duration_ms)); await this.db.query('update local_media_index set thumbnail_status = \'READY\', thumbnail_error = null, updated_at = now() where file_id = $1', [fileId]); return { key, path }; }
    catch (error) { const message = error instanceof Error ? error.message.slice(0, 500) : 'thumbnail generation failed'; const details = JSON.stringify({ code: 'THUMBNAIL_GENERATION_FAILED', message }); await this.db.query('update local_media_index set thumbnail_status = \'FAILED\', thumbnail_error = $2, updated_at = now() where file_id = $1', [fileId, details]); throw error; }
  }
