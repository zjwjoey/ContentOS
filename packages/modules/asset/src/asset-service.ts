...
export interface ImportAssetInput { projectId?: string; workspaceId?: string; sourcePath: string; kind: string; }
...
export class AssetService {
  ...
  async commitPrepared(input: ImportAssetInput, prepared: PreparedAssetImport, transaction?: AssetTransaction): Promise<AssetResult> {
    ...
    if (existing.rows[0]) {
      ...
      else await db.query('insert into video_workspace_assets (workspace_id, asset_id, role) values ($1, $2, $3) on conflict do nothing', [input.workspaceId, String(row.id), 'SOURCE']);
    ...
    else await db.query('insert into video_workspace_assets (workspace_id, asset_id, role) values ($1, $2, $3) on conflict do nothing', [input.workspaceId, id, 'SOURCE']);
