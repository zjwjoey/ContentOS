...
export class FakeAIProvider implements AIProvider {
  ...
  async generateText(request: AIRequest): Promise<AIResult<string>> {
    ...
    return {
      ...
      usage: { inputTokens: request.input.length, outputTokens: 32, totalTokens: request.input.length + 32 },
    };
  }
  ...
  async generateStructured<T>(request: AIRequest): Promise<AIResult<T>> {
      ...
      return {
        ...
        output: {
          scenes: [
            ...
            {
              sceneIndex: 2,
              voiceoverText: '先验证真实需求，再扩大投入。',
              durationHintSeconds: 5,
              visualInstruction: '展示小规模测试',
              assetKeywords: ['测试', '门店'],
            },
          ],
        } as T,
        usage: { inputTokens: request.input.length, outputTokens: 40, totalTokens: request.input.length + 40 },
      };
    }
    if (request.promptKey === 'review.analysis.v1') {
      return {
        ...
        usage: { inputTokens: request.input.length, outputTokens: 48, totalTokens: request.input.length + 48 },
      };
    }
    return {
      requestId: request.requestId,
      ...
      output: {
        title: '先验证，再增长',
        titleCandidates: ['先验证，再增长'],
        coverText: '先验证',
        topicKeywords: ['经营', '验证需求'],
        hook: '很多人第一步就做错了。',
        body: '先用小成本验证真实需求，再决定是否扩大投入。',
        cta: '收藏这条建议。',
      } as T,
      usage: { inputTokens: request.input.length, outputTokens: 40, totalTokens: request.input.length + 40 },
    };
