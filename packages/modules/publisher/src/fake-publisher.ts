...
import { safeProfileKey } from './publisher-paths.js';
import type {
  AuthResult,
  ExternalStateResult,
...
const failureMap: Record<Exclude<FakeOutcome, 'SUCCESS'>, PublisherFailure> = {
  ...
  UNKNOWN_SIDE_EFFECT: {
    ...
    message: 'Fake platform created the post before the response was lost',
  },
  UNKNOWN_NO_SIDE_EFFECT: {
    code: 'UNKNOWN_EXTERNAL_STATE',
    classification: 'RECONCILIATION_REQUIRED',
    message: 'Fake platform did not confirm whether a post was created',
  },
...
export class FakePublisherAdapter implements PublisherAdapter {
  ...
  async publish(_context: PublisherContext, snapshot: PublishSnapshot): Promise<PublishResult> {
    if (this.outcome !== 'SUCCESS') {
      ...
      if (this.outcome === 'BROWSER_CRASH' || this.outcome === 'UNKNOWN_SIDE_EFFECT')
        this.published.set(snapshot.idempotencyKey, this.externalPostId(snapshot.idempotencyKey));
  ...
  private externalPostId(idempotencyKey: string): string {
    return `fake-post-${createHash('sha256').update(idempotencyKey).digest('hex').slice(0, 16)}`;
  }
...
export class FakePublisherService {
  ...
  constructor(
    ...
    private readonly defaultAdapter: PublisherAdapter = new FakePublisherAdapter(),
  ) {}
  profileDirectory(accountId: string): string {
    return join(this.profileRoot, safeProfileKey(accountId));
  }
  ...
  private adapterFor(accountId: string): PublisherAdapter {
    ...
    const adapter =
      this.adapters.size === 0 || !(this.defaultAdapter instanceof FakePublisherAdapter)
        ? this.defaultAdapter
        : new FakePublisherAdapter(this.defaultAdapter.outcome);
