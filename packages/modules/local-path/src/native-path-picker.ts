...
/** Keep the owner topmost so a picker launched by a background API process is
 * brought in front of Edge/ChatGPT instead of opening behind the browser. */
const dialogOwnerStart = "$owner=New-Object System.Windows.Forms.Form; $owner.ShowInTaskbar=$false; $owner.FormBorderStyle='None'; $owner.StartPosition='Manual'; $owner.Size=New-Object System.Drawing.Size(1,1); $owner.Location=New-Object System.Drawing.Point(-10000,-10000); $owner.TopMost=$true; $owner.Show();";
...
export class WindowsNativePathPicker implements NativePathPicker {
  async pickFolder(_options?: { purpose?: LocalPathPurpose }): Promise<{ cancelled: true } | { cancelled: false; path: string }> {
    ...
    const script = "$ErrorActionPreference='Stop'; Add-Type -AssemblyName System.Windows.Forms; Add-Type -AssemblyName System.Drawing; $dialog=New-Object System.Windows.Forms.FolderBrowserDialog; $dialog.Description='选择 ContentOS 文件夹'; $dialog.ShowNewFolderButton=$true; " + dialogOwnerStart + " $result=$dialog.ShowDialog($owner); $owner.Close(); if($result -eq [System.Windows.Forms.DialogResult]::OK){ [Console]::Out.Write($dialog.SelectedPath) }";
  ...
  async pickFile(options?: { purpose?: LocalPathPurpose; filters?: Array<{ name: string; extensions: string[] }> }): Promise<{ cancelled: true } | { cancelled: false; path: string }> {
    ...
    const script = `$ErrorActionPreference='Stop'; Add-Type -AssemblyName System.Windows.Forms; Add-Type -AssemblyName System.Drawing; $dialog=New-Object System.Windows.Forms.OpenFileDialog; $dialog.Title='选择 ContentOS 文件'; $dialog.Filter='${filter}'; $dialog.Multiselect=$false; ${dialogOwnerStart} $result=$dialog.ShowDialog($owner); $owner.Close(); if($result -eq [System.Windows.Forms.DialogResult]::OK){ [Console]::Out.Write($dialog.FileName) }`;
    const path = await runDialog(script);
    return path ? { cancelled: false, path } : { cancelled: true };
  }
}
