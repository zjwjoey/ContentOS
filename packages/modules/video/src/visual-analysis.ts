...
export interface VisualAnalysisProvider { analyzeAssetFrames(input: { assetId: string; framePaths: string[]; model?: string }): Promise<AssetVisualProfileV3>; }
...
export class QwenVisualAnalysisProvider implements VisualAnalysisProvider {
  constructor(private readonly options: { endpoint?: string; apiKey?: string; model?: string; modelVersion?: string; promptVersion?: string } = {}) {}

  async analyzeAssetFrames(input: { assetId: string; framePaths: string[]; model?: string }): Promise<AssetVisualProfileV3> {
    ...
    const response = await fetch(endpoint, { method: 'POST', headers: { authorization: `Bearer ${apiKey}`, 'content-type': 'application/json' }, body: JSON.stringify({ model: input.model || this.options.model || process.env.QWEN_MODEL || 'qwen-vl-max', messages: [{ role: 'user', content: [{ type: 'text', text: prompt }, ...images.map((image) => ({ type: 'image_url', image_url: { url: image } }))] }], temperature: 0, response_format: { type: 'json_object' } }) });
