...
function parseOperation(value: unknown): QuickEditOperation {
  ...
  if (value.type === 'REPLACE_CLIP' || value.type === 'MANUAL_SELECT_CLIP') {
    ...
    const sourceInMs = value.sourceInMs === undefined ? undefined : requireInteger(value.sourceInMs, 'sourceInMs');
    return { type: value.type, clipIndex: requireInteger(value.clipIndex, 'clipIndex'), assetId: value.assetId.trim(), ...(sourceInMs === undefined ? {} : { sourceInMs }) };
  }
  if (value.type === 'LOCK_CLIP' || value.type === 'UNLOCK_CLIP') return { type: value.type, clipIndex: requireInteger(value.clipIndex, 'clipIndex') };
...
export function applyQuickEditOperations(parent: EditManifestV0, operations: QuickEditOperation[], assets: AdjustmentAsset[] = [], candidateAssets: AdjustmentAsset[] = assets): EditManifestV0 {
  ...
  for (const operation of operations) {
    if (operation.type === 'TRIM') {
      ...
      if (candidateAssets.length === 0) throw new Error('Quick Edit REROLL requires available READY video assets');
      ...
      const candidates = candidateAssets.filter((asset) => sameSourceFamily(current.assetId, asset.id) && asset.id !== current.assetId && asset.id !== previous && asset.id !== following && asset.durationMs >= current.durationMs);
      ...
      const pool = leastUsed(candidates.length > 0 ? candidates : candidateAssets.filter((asset) => sameSourceFamily(current.assetId, asset.id) && asset.id !== current.assetId && asset.durationMs >= current.durationMs));
      ...
      if (candidateAssets.length === 0) throw new Error('Quick Edit REMATCH requires available video assets');
      const current = next.timeline[operation.clipIndex]!;
      const ranked = rankAdjustmentAssets(current.sentenceText || '', candidateAssets.filter((asset) => sameSourceFamily(current.assetId, asset.id)));
      const previous = next.timeline[operation.clipIndex - 1]?.assetId;
      const following = next.timeline[operation.clipIndex + 1]?.assetId;
