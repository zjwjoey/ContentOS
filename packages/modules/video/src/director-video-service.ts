...
import type { StoryboardSceneAssetBindingV1 } from '../../../contracts/src/index.js';
...
function renderInputFingerprint(input: {
  videoAssetIds: string[];
  targetDurationMs: number;
  seed: number;
  voiceAssetId?: string;
  ...
  return createHash('sha256')
    .update(
      JSON.stringify({
        ...
        sceneAssetBindings: input.sceneAssetBindings ?? null,
      }),
    )
    .digest('hex')
    .slice(0, 16);
...
export class DirectorVideoService {
  constructor(
    private readonly director: DirectorService | DirectorV1Service,
    private readonly video: VideoService,
    private readonly legacyDirector?: DirectorService,
  ) {}
  ...
  async createVideoJob(projectId: string, options: DirectorVideoOptions = {}): Promise<JobRecord> {
    if ('getCurrentVideoInput' in this.director) {
      ...
      if (current) {
        if (!current.brief || !current.script || current.script.status !== 'ACCEPTED' || !current.storyboard || current.storyboard.status !== 'APPROVED')
        ...
        const targetDurationMs =
          options.targetDurationMs || current.storyboard.scenes.reduce((total, scene) => total + Math.round(scene.durationHintSeconds * 1000), 0);
        const seed = options.seed ?? 1;
        const plannerMode = options.plannerMode ?? 'STORYBOARD_V1';
        if (plannerMode === 'STORYBOARD_V1' && (!options.sceneAssetBindings || options.sceneAssetBindings.length === 0))
          throw new Error('Storyboard scene asset bindings are required for approved Director V1 video jobs');
        const fingerprint = renderInputFingerprint({
          videoAssetIds,
          targetDurationMs,
          seed,
          plannerMode,
          ...(options.sceneAssetBindings ? { sceneAssetBindings: options.sceneAssetBindings } : {}),
          ...(options.voiceAssetId ? { voiceAssetId: options.voiceAssetId } : {}),
          ...(options.subtitleText ? { subtitleText: options.subtitleText } : {}),
        });
        return this.video.createJob({
          ...
          ...(plannerMode === 'STORYBOARD_V1'
            ? {
                sceneAssetBindings: options.sceneAssetBindings || [],
                storyboardScenes: current.storyboard.scenes.map((scene) => ({ sceneIndex: scene.sceneIndex, durationHintSeconds: scene.durationHintSeconds })),
              }
            : {}),
          ...(options.voiceAssetId ? { voiceAssetId: options.voiceAssetId } : {}),
          ...(options.subtitleText ? { subtitleText: options.subtitleText } : {}),
          idempotencyKey: `video-render:director-v1:${projectId}:${current.storyboard.id}:${fingerprint}`,
          metadata: { briefId: current.brief.id, scriptRevisionId: current.script.id, storyboardRevisionId: current.storyboard.id, plannerMode },
        });
      }
  ...
  private async createLegacyVideoJob(projectId: string, options: DirectorVideoOptions, director: DirectorService): Promise<JobRecord> {
    ...
    const input = {
      ...
      ...(options.voiceAssetId ? { voiceAssetId: options.voiceAssetId } : {}),
      ...(options.subtitleText ? { subtitleText: options.subtitleText } : {}),
      idempotencyKey: `video-render:director:${projectId}:${approved.id}:${fingerprint}`,
      directorRevisionId: approved.id,
      directorRevision: approved.revision,
      directorBrief: plan.brief,
      directorStoryboard: plan.storyboard,
    };
