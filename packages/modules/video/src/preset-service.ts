...
export class VideoEditPresetService {
  ...
  private async validateBrandingAsset(assetId: string | null | undefined): Promise<void> {
    if (!assetId) return;
    const result = await this.db.query('select id from assets where id = $1 and project_id is null and kind = $2 and lifecycle = $3', [assetId, 'VIDEO', 'READY']);
    if (!result.rows[0]) throw new Error('PRESET_BRANDING_ASSET_INVALID');
  }
