...
export function buildScriptMontageManifest(input: ScriptMontageInput): SentenceMontageResult {
  ...
  const random = seededRandom(input.seed); const assets = [...input.assets].sort((a, b) => a.id.localeCompare(b.id)); const timeline: EditManifestV0['timeline'] = []; const decisions: SentenceMontageDecision[] = []; const usedAssetIds = new Set<string>(); const randomizeLocalMedia = input.randomizeLocalMedia === true;
  if (input.resolvedAssignments) {
    // Hybrid SCRIPT is PREFER UNIQUE, with reuse allowed only when the resolver explicitly opts in.
    ...
    for (let sentenceIndex = 0; sentenceIndex < sentences.length; sentenceIndex += 1) {
      ...
      const reused = usedAssetIds.has(asset.id);
      if (reused && assignment.allowAssetReuse !== true) throw new Error('EDIT_UNIQUE_MEDIA_EXHAUSTED:同一素材不能在同一任务中重复使用');
       ...
       const placement = visualTiming(sentences, sentence, sentenceIndex, visualCursor); visualCursor = placement.endMs; const matching: ClipMatchingV1 = { matchedKeywords: assignment.matchedKeywords, matchScore: assignment.matchScore ?? (assignment.entityFallback ? 0 : 100), fallback: assignment.fallback ?? assignment.entityFallback, matchingReason: assignment.reason, visualIntent: assignment.visualIntent, selectedSource: assignment.selectedSource, selectedRole: assignment.selectedRole, entityFallback: assignment.entityFallback, ...(assignment.allowAssetReuse ? { allowAssetReuse: true } : {}), ...(assignment.searchQuery ? { query: assignment.searchQuery } : {}), reason: assignment.reason };
  ...
  for (let sentenceIndex = 0; sentenceIndex < sentences.length; sentenceIndex += 1) {
    ...
    const fallbackPool = ranked.filter((item) => item.asset.durationMs >= requestedDuration);
    const selected = randomizeLocalMedia
      ? (eligible[Math.floor(random() * eligible.length)] || fallbackPool[Math.floor(random() * fallbackPool.length)])
      : (eligible[0] || fallbackPool[0]);
...
// MIX / RANDOM is STRICT UNIQUE: exhausted source pools fail instead of silently repeating a clip.
...
export function assembleBrandedTimeline(manifest: EditManifestV0, branding: BrandingConfig): EditManifestV0 {
  ...
  const shiftedTimeline = timeline.map((clip, index) => {
    if (clip.role === 'INTRO') return { ...clip, timelineStartMs: 0, timelineEndMs: clip.durationMs };
    if (clip.role === 'OUTRO') { const start = Math.max(offset, ...content.map((item) => item.timelineEndMs ?? ((item.timelineStartMs ?? 0) + item.durationMs))); return { ...clip, timelineStartMs: start, timelineEndMs: start + clip.durationMs }; }
    const start = (clip.timelineStartMs ?? 0) + offset; return { ...clip, timelineStartMs: start, timelineEndMs: (clip.timelineEndMs ?? ((clip.timelineStartMs ?? 0) + clip.durationMs)) + offset, ...(clip.voiceStartMs !== undefined ? { voiceStartMs: clip.voiceStartMs + offset } : {}), ...(clip.voiceEndMs !== undefined ? { voiceEndMs: clip.voiceEndMs + offset } : {}) };
  });
  const shiftedSentences = manifest.metadata?.sentences?.map((sentence) => ({ ...sentence, ...(sentence.voiceStartMs !== undefined ? { voiceStartMs: sentence.voiceStartMs + offset } : {}), ...(sentence.voiceEndMs !== undefined ? { voiceEndMs: sentence.voiceEndMs + offset } : {}) }));
  return validateAndReturn({ ...manifest, timeline: shiftedTimeline, metadata: { ...(manifest.metadata || {}), preferUnusedMedia: manifest.metadata?.preferUnusedMedia !== false, audioOffsetMs: offset, ...(shiftedSentences ? { sentences: shiftedSentences } : {}) }, ...(manifest.subtitles ? { subtitles: manifest.subtitles.map((subtitle) => ({ ...subtitle, startMs: subtitle.startMs + offset, endMs: subtitle.endMs + offset })) } : {}), ...(manifest.textOverlays ? { textOverlays: manifest.textOverlays.map((overlay) => ({ ...overlay, startMs: overlay.startMs + offset, endMs: overlay.endMs + offset })) } : {}) });
}
