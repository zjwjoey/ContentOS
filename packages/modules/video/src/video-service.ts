...
import { buildStoryboardManifest, buildVideoManifest, type PlannerAsset } from './planner.js';
import { validateEditManifest, type EditManifestV0, type StoryboardSceneAssetBindingV1, type StoryboardSceneV1 } from '../../../contracts/src/index.js';
...
export interface VideoPlanResult {
  manifestId: string;
  renderId: string;
  manifest: ReturnType<typeof buildVideoManifest>;
  renderStatus: string;
...
export class VideoService {
  ...
  private async ensureProjectWorkspace(projectId: string): Promise<void> {
    await this.db.query("insert into video_workspaces (id, type, project_id) values ($1, 'PROJECT', $2) on conflict (project_id) do nothing", [
  ...
  async createJob(input: CreateVideoJobInput): Promise<JobRecord> {
    ...
    try {
      return await this.jobs.create({
        ...
        workspaceId: projectWorkspaceId(input.projectId),
        type: 'VIDEO_RENDER',
        payload: input,
        idempotencyKey,
        maxAttempts: 3,
  ...
  async createManifestRenderJob(projectId: string, manifestId: string): Promise<JobRecord> {
    ...
    const manifest = await this.db.query<{ revision: number; project_id: string; manifest: EditManifestV0; manifest_digest: string | null }>(
      'select revision, project_id, manifest, manifest_digest from edit_manifests where id = $1 and project_id = $2',
    ...
    if (!row.manifest_digest)
      await this.db.query('update edit_manifests set manifest_digest = $2 where id = $1 and manifest_digest is null', [manifestId, manifestDigest]);
  ...
  async createManifestRenderJobForWorkspace(workspaceId: string, manifestId: string): Promise<JobRecord> {
    const result = await this.db.query<{ revision: number; workspace_id: string; manifest: EditManifestV0; manifest_digest: string | null }>(
      'select revision, workspace_id, manifest, manifest_digest from edit_manifests where id = $1 and workspace_id = $2',
      [manifestId, workspaceId],
    );
    ...
    try {
      return await this.jobs.create({
        ...
        workspaceId,
        type: 'VIDEO_RENDER',
        payload: { workspaceId, manifestId, manifestRevision: Number(row.revision), manifestDigest } as VideoJobPayload,
        idempotencyKey,
        maxAttempts: 3,
      });
    } catch (error) {
  ...
  async planJob(job: JobRecord): Promise<VideoPlanResult> {
    ...
    if (!projectId && (job.workspaceId || payload.workspaceId))
      return this.planWorkspaceManifestJob(job, { ...payload, workspaceId: payload.workspaceId || job.workspaceId! });
    ...
    const completed = await this.db.query<{
      ...
      "select r.id as render_id, m.id as manifest_id, m.manifest, r.status as render_status, r.output_asset_id from renders r join edit_manifests m on m.id = r.manifest_id and m.project_id = r.project_id where r.job_id = $1 and r.project_id = $2 and r.status = 'SUCCEEDED' and r.output_asset_id is not null order by r.created_at desc, r.id desc limit 1",
      [job.id, projectId],
    );
    const completedRender = completed.rows[0];
    if (completedRender)
      return {
        manifestId: completedRender.manifest_id,
        renderId: completedRender.render_id,
        manifest: completedRender.manifest,
        renderStatus: completedRender.render_status,
        outputAssetId: completedRender.output_asset_id,
      };
    ...
    const manifest =
      payload.plannerMode === 'STORYBOARD_V1'
        ? buildStoryboardManifest({
            projectId,
            seed: payload.seed,
            scenes: payload.storyboardScenes || [],
            sceneAssetBindings: payload.sceneAssetBindings || [],
            assets: plannerAssets,
            ...(voicePath ? { voicePath } : {}),
            ...(payload.voiceAssetId ? { voiceAssetId: payload.voiceAssetId } : {}),
            ...(payload.metadata ? { metadata: payload.metadata } : {}),
          })
        : buildVideoManifest({ ...payload, projectId, assets: plannerAssets, ...(voicePath ? { voicePath } : {}) });
    ...
    try {
      ...
      const existing = await client.query<{
        ...
        'select r.id as render_id, m.id as manifest_id, m.manifest, r.status as render_status, r.output_asset_id from renders r join edit_manifests m on m.id = r.manifest_id and m.project_id = r.project_id where r.job_id = $1 and r.project_id = $2 order by r.created_at desc, r.id desc limit 1',
        [job.id, projectId],
      );
      ...
      if (prior) {
        ...
        return {
          ...
          renderId: prior.render_id,
          manifest: prior.manifest,
          renderStatus: prior.render_status,
          outputAssetId: prior.output_asset_id,
        };
      }
      ...
      const revisionResult = await client.query<{ revision: number }>(
        'select coalesce(max(revision), 0) + 1 as revision from edit_manifests where project_id = $1',
        [projectId],
      );
      const revision = Number(revisionResult.rows[0]?.revision || 1);
      const manifestId = `manifest-${randomUUID()}`;
      await client.query(
        'insert into edit_manifests (id, project_id, workspace_id, revision, schema_version, manifest, manifest_digest, status) values ($1, $2, $3, $4, $5, $6, $7, $8)',
        [manifestId, projectId, projectWorkspaceId(projectId), revision, 'EDIT_MANIFEST_V0', manifest, digestEditManifest(manifest), 'PERSISTED'],
      );
      const renderId = `render-${randomUUID()}`;
      await client.query('insert into renders (id, project_id, workspace_id, manifest_id, job_id, status, diagnostics) values ($1, $2, $3, $4, $5, $6, $7)', [
        renderId,
        projectId,
        projectWorkspaceId(projectId),
        manifestId,
        job.id,
        'QUEUED',
        { seed: payload.seed, ...(payload.metadata ? { metadata: payload.metadata } : {}) },
      ]);
      await client.query('commit');
  ...
  private async planManifestJob(job: JobRecord, payload: VideoJobPayload): Promise<VideoPlanResult> {
    ...
    const selected = await this.db.query<{ id: string; project_id: string; revision: number; manifest: EditManifestV0; manifest_digest: string | null }>(
      'select id, project_id, revision, manifest, manifest_digest from edit_manifests where id = $1 and project_id = $2',
      [payload.manifestId, job.projectId],
    );
    ...
    if (payload.manifestDigest !== currentDigest || (row.manifest_digest && row.manifest_digest !== currentDigest))
      throw new Error('VIDEO_MANIFEST_DIGEST_CONFLICT');
    ...
    try {
      ...
      await client.query('insert into renders (id, project_id, workspace_id, manifest_id, job_id, status, diagnostics) values ($1, $2, $3, $4, $5, $6, $7)', [
        renderId,
        job.projectId,
        projectWorkspaceId(job.projectId!),
        payload.manifestId,
        job.id,
        'QUEUED',
  ...
  private async planWorkspaceManifestJob(job: JobRecord, payload: VideoJobPayload): Promise<VideoPlanResult> {
    if (!payload.manifestId || !Number.isInteger(payload.manifestRevision) || !payload.manifestDigest || !payload.workspaceId)
      throw new Error('VIDEO_MANIFEST_SCOPE_REQUIRED');
    if (!this.storage || !this.assets) throw new Error('Video storage and asset catalog are required for exact Manifest rendering');
    const selected = await this.db.query<{ id: string; workspace_id: string; revision: number; manifest: EditManifestV0; manifest_digest: string | null }>(
      'select id, workspace_id, revision, manifest, manifest_digest from edit_manifests where id = $1 and workspace_id = $2',
      [payload.manifestId, payload.workspaceId],
    );
    const row = selected.rows[0];
    if (!row) throw new Error('VIDEO_MANIFEST_NOT_FOUND');
    if (Number(row.revision) !== payload.manifestRevision) throw new Error('VIDEO_MANIFEST_REVISION_CONFLICT');
    ...
    const existing = await this.db.query<{
      ...
      'select r.id as render_id, r.manifest_id, m.manifest, r.status as render_status, r.output_asset_id from renders r join edit_manifests m on m.id = r.manifest_id and m.workspace_id = r.workspace_id where r.job_id = $1 and r.workspace_id = $2 order by r.created_at desc, r.id desc limit 1',
      [job.id, payload.workspaceId],
    );
    const prior = existing.rows[0];
    if (prior)
      return {
    ...
    const renderId = `render-${randomUUID()}`;
    await this.db.query('insert into renders (id, project_id, workspace_id, manifest_id, job_id, status, diagnostics) values ($1, null, $2, $3, $4, $5, $6)', [
      renderId,
      payload.workspaceId,
      payload.manifestId,
      job.id,
  ...
  async startRender(renderId: string, attempt: JobAttemptScope, diagnostics: unknown = {}): Promise<boolean> {
    const result = await attempt.query(
      "update renders set status = 'RUNNING', attempt_id = $2, attempt_number = $3, diagnostics = $4, finished_at = null where id = $1 and job_id = $5 and status not in ('SUCCEEDED', 'CANCELLED') and (attempt_number is null or attempt_number < $3 or (attempt_number = $3 and attempt_id = $2)) returning id",
      [renderId, attempt.attemptId, attempt.attemptNumber, diagnostics, attempt.jobId],
    );
  ...
  async completeRender(renderId: string, attempt: JobAttemptScope, outputAssetId: string, diagnostics: unknown): Promise<boolean> {
    const result = await attempt.query(
      "update renders set status = 'SUCCEEDED', output_asset_id = $3, diagnostics = $4, finished_at = now() where id = $1 and job_id = $5 and attempt_id = $2 and attempt_number = $6 and status = 'RUNNING' returning id",
      [renderId, attempt.attemptId, outputAssetId, diagnostics, attempt.jobId, attempt.attemptNumber],
    );
    return Boolean(result.rowCount);
  }
  ...
  async failRender(renderId: string, attempt: JobAttemptScope, diagnostics: unknown): Promise<boolean> {
    const result = await attempt.query(
      "update renders set status = 'FAILED', diagnostics = $3, finished_at = now() where id = $1 and job_id = $4 and attempt_id = $2 and attempt_number = $5 and status = 'RUNNING' returning id",
      [renderId, attempt.attemptId, diagnostics, attempt.jobId, attempt.attemptNumber],
    );
    return Boolean(result.rowCount);
  }
  ...
  async cancelRender(renderId: string, attempt: JobAttemptScope, diagnostics: unknown): Promise<boolean> {
    const result = await attempt.query(
      "update renders set status = 'CANCELLED', diagnostics = $3, finished_at = now() where id = $1 and job_id = $4 and attempt_id = $2 and attempt_number = $5 and status = 'RUNNING' returning id",
      [renderId, attempt.attemptId, diagnostics, attempt.jobId, attempt.attemptNumber],
    );
    return Boolean(result.rowCount);
  }
  ...
  async cancelCurrentRender(attempt: JobAttemptScope, diagnostics: unknown): Promise<number> {
    const result = await attempt.query(
      "update renders set status = 'CANCELLED', diagnostics = $2, finished_at = now() where job_id = $1 and attempt_id = $3 and attempt_number = $4 and status = 'RUNNING' returning id",
      [attempt.jobId, diagnostics, attempt.attemptId, attempt.attemptNumber],
    );
