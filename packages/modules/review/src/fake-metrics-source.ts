...
export class FakeMetricsSource implements ReviewMetricsSource {
  ...
  async collect(post: PublisherExternalPost): Promise<CollectedReviewMetrics> {
    if (this.outcome === 'UNAVAILABLE')
      throw Object.assign(new Error('Fake metrics source unavailable'), { code: 'METRICS_SOURCE_UNAVAILABLE', retryable: true });
    if (this.outcome === 'MALFORMED')
      throw Object.assign(new Error('Fake metrics source returned malformed data'), { code: 'METRICS_SOURCE_MALFORMED', retryable: false });
    ...
    return {
      ...
      metrics: {
        plays,
        likes: Math.floor(plays * 0.08),
        comments: Math.floor(plays * 0.01),
        saves: Math.floor(plays * 0.02),
        shares: Math.floor(plays * 0.005),
      },
