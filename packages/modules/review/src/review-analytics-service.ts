...
import { validateMetricSnapshotV1, validateReviewAnalysisReportV1, type MetricSnapshotV1, type ReviewAnalysisReportV1 } from '../../../contracts/src/index.js';
...
export class ReviewAnalyticsService {
  ...
  async listMetricSnapshots(projectId: string, externalPostId: string): Promise<MetricSnapshotV1[]> {
    const result = await this.db.query(
      'select * from review_metric_snapshots where project_id = $1 and external_post_id = $2 order by captured_at desc, id desc',
  ...
  async getMetricSnapshots(projectId: string, externalPostId: string, ids: string[]): Promise<MetricSnapshotV1[]> {
    ...
    const result = await this.db.query(
      'select * from review_metric_snapshots where project_id = $1 and external_post_id = $2 and id = any($3::text[]) order by captured_at asc, id asc',
      [projectId, externalPostId, ids],
    );
    return result.rows.map((row) => mapSnapshot(row as Record<string, unknown>));
  }
  ...
  async listAnalysisReports(projectId: string, externalPostId: string): Promise<ReviewAnalysisReportV1[]> {
    const result = await this.db.query(
      'select * from review_analysis_reports where project_id = $1 and external_post_id = $2 order by created_at desc, id desc',
      [projectId, externalPostId],
    );
