...
export class BenchmarkService {
  ...
  async attach(projectId: string, contentId: string): Promise<void> { const result = await this.db.query('insert into benchmark_references (project_id, benchmark_content_id) select $1, id from benchmark_contents where id = $2 and project_id = $1 on conflict do nothing', [projectId, contentId]); if (!result.rowCount) throw new Error('Benchmark content not found for project'); }
  ...
  async recordAnalysis(input: Omit<BenchmarkAnalysisV1, 'schemaVersion'>): Promise<BenchmarkAnalysisV1> { const analysis: BenchmarkAnalysisV1 = { schemaVersion: 'BENCHMARK_ANALYSIS_V1', ...input }; validateBenchmarkAnalysisV1(analysis); const result = await this.db.query('insert into benchmark_analyses (id, project_id, benchmark_content_id, ai_run_id, analysis, created_at) values ($1,$2,$3,$4,$5,$6) returning *', [analysis.id, analysis.projectId, analysis.benchmarkContentId, analysis.aiRunId, { ...analysis, schemaVersion: undefined, id: undefined, projectId: undefined, benchmarkContentId: undefined, aiRunId: undefined, createdAt: undefined } as unknown, analysis.createdAt]); return mapAnalysis(result.rows[0] as Record<string, unknown>); }
