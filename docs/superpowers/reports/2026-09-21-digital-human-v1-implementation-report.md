...
- HZAgent runtime selection now also fails closed when an API key is present but the endpoint and authentication mapping is not explicitly configured; the example environment no longer presents unverified vendor paths as defaults.
- The shared config loader now preserves an empty HZAgent auth scheme by default, so the local Worker composition cannot silently reintroduce a Bearer scheme; an empty auth header is also treated as an incomplete mapping.
