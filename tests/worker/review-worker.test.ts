...
test('Fake metrics source is deterministic in values and classifies unavailable as retryable', async () => {
  ...
  await assert.rejects(
    () => new FakeMetricsSource('UNAVAILABLE').collect(post),
...
test('Review worker rejects malformed ownership payloads before collecting', async () => {
  const handler = createReviewJobHandler({
    jobs: {} as never,
    analytics: {} as never,
    posts: { getExternalPost: async () => null },
  ...
  await assert.rejects(
      ...
      handler(
        {
          ...
          state: 'RUNNING',
          payload: { schemaVersion: 'bad', projectId: 'other' },
          result: null,
          error: null,
          attemptCount: 1,
          maxAttempts: 3,
    ...
    /Invalid Review metric collection/,
  );
});
...
test('Review worker rejects incomplete snapshot sets and persists validated analysis reports', async () => {
    ...
    getMetricSnapshots: async () => [
      {
        ...
        publishedAt: null,
        metrics: { plays: 100, likes: 10, comments: 1, saves: 2, shares: 1 },
        source: 'FAKE',
        sourceReference: 'fake',
        createdAt: '2026-08-30T12:00:00.000Z',
      },
    ],
    recordAnalysisReport: async (report: unknown) => {
      reports.push(report);
      return report;
    },
  };
  const jobs = {
    succeedWithCurrentAttempt: async (_id: string, _attempt: string, action: () => Promise<unknown>) => ({ executed: true, value: await action(), job: {} }),
  };
  const posts = {
    getExternalPost: async () => ({
      id: 'post',
      requestId: 'request',
      accountId: 'account',
      platformId: 'fake-platform',
  ...
  const ai = {
    generateStructured: async (_input: unknown, validator: (value: unknown) => unknown) => ({
      aiRunId: 'ai-run',
      output: validator({
        summary: 'summary',
        highlights: [{ title: 'h', detail: 'd' }],
        risks: [],
        recommendations: [{ priority: 'LOW', title: 'r', detail: 'd' }],
      }),
    }),
  };
  const handler = createReviewJobHandler({
