...
test('migration files form a complete ordered chain through 0032', async () => {
  const names = (await readdir(resolveMigrationsDirectory())).filter((file) => /^\d+_.+\.sql$/.test(file) && !file.endsWith('.down.sql')).sort();
  assert.deepEqual(names.map((file) => file.slice(0, 4)), Array.from({ length: 32 }, (_, index) => String(index + 1).padStart(4, '0')));
});
