...
const migrationNames = readdirSync(migrationDirectory)
  .filter((entry) => /^\d+_.+\.sql$/u.test(entry) && !entry.endsWith('.down.sql'))
...
for (const [label, subset] of [['clean', 0], ['0001-0005', 5], ['0001-0006', 6]] as const) {
  test(`migration matrix applies full chain from ${label}`, async () => {
    ...
    try {
      ...
      assert.equal(result.applied, migrationNames.length - subset);
...
test('migration 0027 down normalizes live preparation/rendering states and can migrate up again', async () => {
  ...
  try {
    ...
    try {
      ...
      const downToWorkbenchHardening = migrationNames.filter((name) => Number(name) >= 27).length;
      for (let index = 0; index < downToWorkbenchHardening; index += 1) assert.equal((await migrateDown(db)).removed, 1);
      ...
      assert.equal((await migrateUp(db)).applied, downToWorkbenchHardening);
