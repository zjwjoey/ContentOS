...
test('Digital Human Worker imports a remote avatar result into a project Asset', async () => {
  ...
  try {
    ...
    await generateFixtureVideo(join(root, 'clip.mp4'), ffmpegPath, 'blue', 6); await generateFixtureVideo(join(root, 'remote.mp4'), ffmpegPath, 'green', 5); await generateFixtureAudio(join(root, 'speech.wav'), ffmpegPath);
