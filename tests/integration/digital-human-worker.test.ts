...
test('Digital Human Worker imports a remote avatar result into a project Asset', async () => {
  ...
  try {
    ...
    await generateFixtureVideo(join(root, 'clip.mp4'), ffmpegPath, 'blue', 6); await generateFixtureVideo(join(root, 'remote.mp4'), ffmpegPath, 'green', 2); await generateFixtureAudio(join(root, 'speech.wav'), ffmpegPath);
    ...
    const maliciousProviderMetadata = { provider: 'evil-provider', externalTaskId: 'evil-task', sourceVideoAssetId: 'evil-asset', costAmount: 0, probe: { width: 1 }, marker: 'preserve-raw' };
    const avatarProvider: AvatarProvider = { providerId: 'test-avatar', getCapabilities: async () => ({ providerId: 'test-avatar', local: false, videoToVideo: true, imageToVideo: false, requiresPublicUrl: true, supportedFormats: ['mp4'] }), submitLipSync: async () => ({ externalTaskId: 'remote-worker-1', providerId: 'test-avatar', status: 'SUCCEEDED', outputUrl: 'https://provider.test/avatar.mp4', model: 'test-model', modelVersion: 'test-1', costAmount: 3.15, costCurrency: 'RMB', billingQuantity: 63, billingUnit: 'second', provenance: maliciousProviderMetadata }), getTask: async () => ({ externalTaskId: 'remote-worker-1', providerId: 'test-avatar', status: 'SUCCEEDED', outputUrl: 'https://provider.test/avatar.mp4', costAmount: 3.15, costCurrency: 'RMB', billingQuantity: 63, billingUnit: 'second', provenance: maliciousProviderMetadata }) };
    const result = await new JobRunner(jobs, 'digital-human-worker-integration').run(created.job.id, createDigitalHumanJobHandler({ jobs, digitalHuman, assets: catalog, assetService: assets, probeRemoteResult: (path, signal) => probeMedia(path, ffprobePath, signal), storage, speechProvider: {} as never, avatarProvider, staging: { stageAsset: async (assetId) => ({ publicUrl: `https://staging.test/${assetId}`, expiresAt: new Date(Date.now() + 60_000).toISOString() }) }, fetchImpl: async (input, init) => fetch(String(input).replace('https://provider.test', `http://127.0.0.1:${address.port}`), init), resolveRemoteMedia: async () => [{ address: '93.184.216.34', family: 4 }] }));
    ...
    const output = await catalog.getProjectAsset(project.id, generation!.outputAssetId!); assert.equal(output?.kind, 'VIDEO'); assert.equal(output?.lifecycle, 'READY'); const outputMetadata = output?.metadata as Record<string, unknown> | undefined; const outputDigitalHuman = outputMetadata?.digitalHuman as Record<string, unknown> | undefined; const outputProviderMetadata = outputDigitalHuman?.providerMetadata as Record<string, unknown> | undefined; const provenance = generation?.provenance as Record<string, unknown> | undefined; const probe = provenance?.probe as Record<string, unknown> | undefined; const providerMetadata = provenance?.providerMetadata as Record<string, unknown> | undefined; assert.equal(outputMetadata?.format, 'mp4'); assert.equal(outputMetadata?.width, 640); assert.equal(outputMetadata?.height, 360); assert.ok(Number(outputMetadata?.durationMs) > 0); assert.equal(probe?.width, 640); assert.equal(probe?.height, 360); assert.equal(provenance?.provider, 'test-avatar'); assert.equal(provenance?.externalTaskId, 'remote-worker-1'); assert.equal(provenance?.sourceVideoAssetId, clipAsset.id); assert.equal(provenance?.costAmount, 3.15); assert.equal(providerMetadata?.provider, 'evil-provider'); assert.equal(providerMetadata?.externalTaskId, 'evil-task'); assert.equal(providerMetadata?.sourceVideoAssetId, 'evil-asset'); assert.equal((providerMetadata?.probe as Record<string, unknown>)?.width, 1); assert.equal(outputProviderMetadata?.marker, 'preserve-raw');
