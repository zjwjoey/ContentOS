...
test('Digital Human Worker imports a remote avatar result into a project Asset', async () => {
  ...
  try {
    ...
    const result = await new JobRunner(jobs, 'digital-human-worker-integration').run(created.job.id, createDigitalHumanJobHandler({ jobs, digitalHuman, assets: catalog, assetService: assets, storage, speechProvider: {} as never, avatarProvider, staging: { stageAsset: async (assetId) => ({ publicUrl: `https://staging.test/${assetId}`, expiresAt: new Date(Date.now() + 60_000).toISOString() }) }, fetchImpl: async (input, init) => fetch(String(input).replace('https://provider.test', `http://127.0.0.1:${address.port}`), init), resolveRemoteMedia: async () => [{ address: '93.184.216.34', family: 4 }] }));
