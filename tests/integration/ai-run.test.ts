...
test('AIService persists a safe successful AI Run with prompt/model provenance', async () => {
  ...
  const projectId = 'project-ai-run-001';
  const jobId = 'job-ai-run-001';
  const attemptId = 'attempt-ai-run-001';
  try {
    await migrateUp(db);
    await db.query('delete from ai_runs where project_id = $1', [projectId]);
    await db.query('delete from job_attempts where id = $1', [attemptId]);
    ...
    await db.query('insert into job_attempts (id, job_id, attempt_number, worker_id, status) values ($1, $2, 1, $3, $4)', [
      attemptId,
      jobId,
      'test-worker',
      'RUNNING',
    ...
    const result = await service.generateText({
      ...
      jobId,
      attemptId,
      correlationId: 'corr-ai-run-001',
      operation: 'DIRECTOR_GENERATE_SCRIPT',
      promptKey: 'director.script.v1',
      variables: { topic: '门店经营', coreThesis: '先验证，再扩大投入。' },
    });
    ...
    }>('select provider_id, model_profile_id, prompt_version_id, status, input_snapshot from ai_runs where id = $1', [result.aiRunId]);
    const catalog = await db.query<{ id: string }>('select id from ai_model_profiles where provider_id = $1 and model_id = $2', [
      profile.providerId,
      profile.modelId,
    ]);
    ...
    await cleanupFixture(db, projectId, jobId, attemptId);
    await db.end();
  }
});
...
test('AIService rejects invalid structured output and records a failed AI Run', async () => {
  const db = await createDatabase(databaseUrl);
  const projectId = 'project-ai-run-002';
  const jobId = 'job-ai-run-002';
  const attemptId = 'attempt-ai-run-002';
  try {
    await migrateUp(db);
    await db.query('delete from ai_runs where project_id = $1', [projectId]);
    await db.query('delete from job_attempts where id = $1', [attemptId]);
      ...
      service.generateStructured(
        {
          ...
          jobId,
          attemptId,
          correlationId: 'corr-ai-run-002',
          operation: 'DIRECTOR_GENERATE_STORYBOARD',
          promptKey: 'director.storyboard.v1',
          variables: { topic: '门店经营', coreThesis: '先验证，再扩大投入。' },
        },
        () => {
...
test('AIService persists Review analysis operation provenance', async () => {
  const db = await createDatabase(databaseUrl);
  const projectId = 'project-ai-run-review-001';
  const jobId = 'job-ai-run-review-001';
  const attemptId = 'attempt-ai-run-review-001';
  try {
    await migrateUp(db);
    await db.query('delete from ai_runs where project_id = $1', [projectId]);
    await db.query('delete from job_attempts where id = $1', [attemptId]);
    ...
    const result = await service.generateStructured(
      {
        ...
        jobId,
        attemptId,
        correlationId: 'corr-review-ai',
        operation: 'REVIEW_GENERATE_ANALYSIS',
        promptKey: 'review.analysis.v1',
        variables: { platformId: 'fake-platform', publishedAt: '2026-08-30T12:00:00.000Z', metrics: '{"plays":100}', history: '[]' },
      },
      (value) => value,
    );
    assert.ok(result.aiRunId);
    const row = await db.query<{ operation: string; prompt_version_id: string }>('select operation, prompt_version_id from ai_runs where id = $1', [
      result.aiRunId,
    ]);
