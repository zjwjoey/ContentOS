...
test('Avatar output enters the existing EditManifest and VIDEO_RENDER path idempotently', async () => {
  ...
  try {
    ...
    const voicePatch = await app.inject({ method: 'PATCH', url: `/api/v1/projects/${project.id}/digital-human/voices/${voiceId}`, payload: { defaultSpeed: 1.1, defaultEmotion: 'natural' } }); assert.equal(voicePatch.statusCode, 200, voicePatch.body); assert.equal(voicePatch.json<{ defaultSpeed: number }>().defaultSpeed, 1.1);
    const voiceDisable = await app.inject({ method: 'DELETE', url: `/api/v1/projects/${project.id}/digital-human/voices/${voiceId}` }); assert.equal(voiceDisable.statusCode, 200, voiceDisable.body); assert.equal(voiceDisable.json<{ status: string }>().status, 'DISABLED');
    const voiceEnable = await app.inject({ method: 'PATCH', url: `/api/v1/projects/${project.id}/digital-human/voices/${voiceId}`, payload: { status: 'READY' } }); assert.equal(voiceEnable.statusCode, 200, voiceEnable.body);
    const avatarPatch = await app.inject({ method: 'PATCH', url: `/api/v1/projects/${project.id}/digital-human/avatars/${avatarProfileId}`, payload: { ownerName: 'updated-owner' } }); assert.equal(avatarPatch.statusCode, 200, avatarPatch.body); assert.equal(avatarPatch.json<{ ownerName: string }>().ownerName, 'updated-owner');
    const clipPatch = await app.inject({ method: 'PATCH', url: `/api/v1/projects/${project.id}/digital-human/avatar-clips/${avatarClipId}`, payload: { tags: ['front', 'ready'] } }); assert.equal(clipPatch.statusCode, 200, clipPatch.body); assert.deepEqual(clipPatch.json<{ tags: string[] }>().tags, ['front', 'ready']);
