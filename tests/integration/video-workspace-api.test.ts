...
test('Video workspace API exposes safe reads, explicit source selection, and project-scoped cancel', async () => {
  ...
  try {
    ...
    await db.query('insert into assets (id, project_id, kind, checksum, byte_size, storage_key, lifecycle, metadata) values ($1, $2, $3, $4, $5, $6, $7, $8)', [sourceAssetId, project.id, 'VIDEO', `sha256:${sourceAssetId}`, 100, `source/${sourceAssetId}.mp4`, 'READY', { durationMs: 1000, width: 1080, height: 1920, format: 'mp4' }]);
    await db.query('insert into project_assets (project_id, asset_id, role) values ($1, $2, $3)', [project.id, sourceAssetId, 'SOURCE']);
    const brief = await director.createBrief(project.id, { topic: '主题', targetPlatform: 'douyin', channelPositioning: '栏目', targetDurationSeconds: 30, contentType: 'knowledge', audience: '运营', coreThesis: '观点', tone: '清晰', referenceMaterial: '材料', mustInclude: ['事实'], mustAvoid: ['夸大'], requirements: {}, createdBy: 'operator' });
    ...
    const storyboard = await director.createStoryboardRevision(project.id, storyboardAggregate.id, { origin: 'MANUAL', scriptRevisionId: accepted.id, scenes: [{ sceneIndex: 1, voiceoverText: '旁白', durationHintSeconds: 1, visualInstruction: '画面', assetKeywords: ['主题'] }], createdBy: 'operator' });
    await director.approveStoryboard(project.id, storyboard.id);

    const created = await app.inject({ method: 'POST', url: `/api/v1/projects/${project.id}/video/jobs`, payload: { videoAssetIds: [sourceAssetId], targetDurationMs: 1000, seed: 7 } });
    ...
    assert.deepEqual(snapshot.json().sourceAssets[0], { id: sourceAssetId, kind: 'VIDEO', lifecycle: 'READY', byteSize: 100, checksum: `sha256:${sourceAssetId}`, originalName: `${sourceAssetId}.mp4`, metadata: { durationMs: 1000, width: 1080, height: 1920, format: 'mp4' } });
