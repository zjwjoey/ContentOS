...
test('approved Director V1 pair preserves provenance in the Video Job payload', async () => {
  ...
  await migrateUp(db);
  const project = await new ProjectService(db).create('Director Video V1');
  ...
  try {
    const createdBrief = await director.createBrief(project.id, brief);
    const aggregate = await director.createScript(project.id, createdBrief.id);
    const draft = await director.createScriptRevision(project.id, aggregate.id, script);
    const accepted = await director.acceptScript(project.id, draft.id);
    const board = await director.createStoryboard(project.id);
    const boardDraft = await director.createStoryboardRevision(project.id, board.id, {
      origin: 'MANUAL',
      scriptRevisionId: accepted.id,
      scenes,
      createdBy: 'editor',
    });
    const approved = await director.approveStoryboard(project.id, boardDraft.id);
    const job = await bridge.createVideoJob(project.id, {
      videoAssetIds: ['asset-video-1'],
      sceneAssetBindings: [{ sceneIndex: 1, assetIds: ['asset-video-1'] }],
    ...
    assert.deepEqual(payload.videoAssetIds, ['asset-video-1']);
    assert.equal(payload.metadata.briefId, createdBrief.id);
    assert.equal(payload.metadata.scriptRevisionId, accepted.id);
    assert.equal(payload.metadata.storyboardRevisionId, approved.id);
  } finally {
...
test('Director V1 Video Job idempotency changes when render inputs change', async () => {
  const db = await createDatabase(databaseUrl);
  await migrateUp(db);
  const project = await new ProjectService(db).create('Director Video V1 Idempotency');
  const director = new DirectorV1Service(db);
  const video = new VideoService(db, new JobService(db));
  const bridge = new DirectorVideoService(director, video);
  try {
    ...
    const first = await bridge.createVideoJob(project.id, {
      videoAssetIds: ['asset-video-1'],
      sceneAssetBindings: [{ sceneIndex: 1, assetIds: ['asset-video-1'] }],
      targetDurationMs: 3_000,
      seed: 1,
    });
    const changed = await bridge.createVideoJob(project.id, {
      videoAssetIds: ['asset-video-2'],
      sceneAssetBindings: [{ sceneIndex: 1, assetIds: ['asset-video-2'] }],
      targetDurationMs: 3_000,
      seed: 1,
    });
...
test('Director V1 Video bridge rejects a draft or incomplete current pair', async () => {
  const db = await createDatabase(databaseUrl);
  await migrateUp(db);
  const project = await new ProjectService(db).create('Director Video V1 Draft');
  const director = new DirectorV1Service(db);
  const bridge = new DirectorVideoService(director, new VideoService(db, new JobService(db)));
  try {
    const createdBrief = await director.createBrief(project.id, brief);
    const aggregate = await director.createScript(project.id, createdBrief.id);
    await director.createScriptRevision(project.id, aggregate.id, script);
