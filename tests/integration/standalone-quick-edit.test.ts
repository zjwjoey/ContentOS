...
test('Standalone Quick Edit plans and adjusts without creating a Content Project', async () => {
  ...
  const assetIds = [`standalone-a-${suffix}`, `standalone-b-${suffix}`, `standalone-c-${suffix}`]; const voiceId = `standalone-voice-${suffix}`; let workspaceId = '';
  try {
    ...
    await db.query('delete from assets where id = any($1::text[])', [assetIds.concat(voiceId)]); await db.end();
  }
});
