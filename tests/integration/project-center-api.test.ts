...
test('Project Center follows Director V1 current targets and ignores superseded approvals', async () => {
  ...
  try {
    ...
    await approvals.create({ projectId: project.id, targetType: 'SCRIPT', targetId: scriptAggregate.id, targetRevisionId: firstScript.id, status: 'REJECTED', approver: 'operator', reason: '旧版本' });
    await approvals.create({ projectId: project.id, targetType: 'STORYBOARD', targetId: firstBoardAggregate.id, targetRevisionId: firstBoard.id, status: 'REJECTED', approver: 'operator', reason: '旧版本' });
    ...
    await approvals.create({ projectId: project.id, targetType: 'SCRIPT', targetId: scriptAggregate.id, targetRevisionId: secondScript.id, status: 'APPROVED', approver: 'operator' });
    await approvals.create({ projectId: project.id, targetType: 'STORYBOARD', targetId: secondBoardAggregate.id, targetRevisionId: secondBoard.id, status: 'APPROVED', approver: 'operator' });
