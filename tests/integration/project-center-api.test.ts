...
test('Project Center maps Director, failed Video Job, pending Approval and Publisher human action', async () => {
  ...
  try {
    ...
    const pending = await approvals.create({ projectId: project.id, targetType: 'RENDER', targetId: 'render-legacy-' + project.id, targetRevisionId: revision.id, status: 'PENDING', approver: 'operator' });
    assert.equal(pending.status, 'PENDING');
    const afterPending = await app.inject({ method: 'GET', url: '/api/v1/projects/' + project.id + '/center' });
    assert.equal(afterPending.json().actions.some((action: { kind: string; id: string }) => action.kind === 'APPROVAL' && action.id.includes('RENDER')), false);
...
test('Project Center reports a confirmed Publisher post as complete', async () => {
  ...
  try {
    ...
    await approvals.create({ projectId: project.id, targetType: 'PUBLISH', targetId: request.request.id, targetRevisionId: request.revision.id, status: 'PENDING', approver: 'operator' });
...
test('Project Center only evaluates approval for the current Publisher revision', async () => {
  ...
  try {
    ...
    await db.query('insert into assets (id, project_id, kind, checksum, byte_size, storage_key, lifecycle, metadata) values ($1, $2, $3, $4, $5, $6, $7, $8)', [assetId, project.id, 'VIDEO_RENDER', 'sha256:approval', 100, 'renders/' + assetId + '.mp4', 'READY', { width: 1080, height: 1920 }]);
    const account = await publisher.createAccount({ projectId: project.id, platformId: 'fake-platform', displayName: 'Fake Approval', credentialRef: 'server-ref', profileKey: 'server-profile', status: 'READY', capabilitySnapshot: { platformId: 'fake-platform', mediaTypes: ['video/mp4'], scheduling: false, requiresHumanConfirmation: false } });
    const request = await publisher.createRequest({ projectId: project.id, accountId: account.id, idempotencyKey: 'center-approval-' + randomUUID(), correlationId: 'center-approval-correlation-' + randomUUID(), revision: { assetId, assetChecksum: 'sha256:approval', title: '旧版本', description: '', desiredPublishAt: null, createdBy: 'operator' } });
    await approvals.create({ projectId: project.id, targetType: 'PUBLISH', targetId: request.request.id, targetRevisionId: request.revision.id, status: 'PENDING', approver: 'operator' });
    await approvals.reject(project.id, 'PUBLISH', request.request.id, request.revision.id, 'operator', '需要修改');
    const currentRevision = await publisher.addRevision(request.request.id, { assetId, assetChecksum: 'sha256:approval', title: '新版本', description: '', desiredPublishAt: null, createdBy: 'operator' });
    await approvals.create({ projectId: project.id, targetType: 'PUBLISH', targetId: request.request.id, targetRevisionId: currentRevision.id, status: 'PENDING', approver: 'operator' });
    ...
    assert.equal(response.json().stages[2].status, 'COMPLETE');
    const pendingRequest = await publisher.createRequest({ projectId: project.id, accountId: account.id, idempotencyKey: 'center-approval-pending-' + randomUUID(), correlationId: 'center-approval-pending-correlation-' + randomUUID(), revision: { assetId, assetChecksum: 'sha256:approval', title: '待审批版本', description: '', desiredPublishAt: null, createdBy: 'operator' } });
    await approvals.create({ projectId: project.id, targetType: 'PUBLISH', targetId: pendingRequest.request.id, targetRevisionId: pendingRequest.revision.id, status: 'PENDING', approver: 'operator' });
    const mixedResponse = await app.inject({ method: 'GET', url: '/api/v1/projects/' + project.id + '/center' });
    assert.equal(mixedResponse.json().stages[2].status, 'ACTION_REQUIRED');
...
test('Project Center follows Director V1 current targets and ignores superseded approvals', async () => {
  ...
  try {
    ...
    const firstScript = await director.createScriptRevision(project.id, scriptAggregate.id, { origin: 'MANUAL', title: '旧脚本', titleCandidates: ['旧脚本'], coverText: '旧', topicKeywords: ['主题'], hook: '开头', body: '正文', createdBy: 'operator' });
    ...
    const firstBoard = await director.createStoryboardRevision(project.id, firstBoardAggregate.id, { origin: 'MANUAL', scriptRevisionId: firstScript.id, scenes: [{ sceneIndex: 1, voiceoverText: '旧', durationHintSeconds: 2, visualInstruction: '旧画面', assetKeywords: ['旧'] }], createdBy: 'operator' });
    await director.approveStoryboard(project.id, firstBoard.id);
    await approvals.create({ projectId: project.id, targetType: 'SCRIPT', targetId: scriptAggregate.id, targetRevisionId: firstScript.id, status: 'PENDING', approver: 'operator' });
    await approvals.reject(project.id, 'SCRIPT', scriptAggregate.id, firstScript.id, 'operator', '旧版本');
    await approvals.create({ projectId: project.id, targetType: 'STORYBOARD', targetId: firstBoardAggregate.id, targetRevisionId: firstBoard.id, status: 'PENDING', approver: 'operator' });
    await approvals.reject(project.id, 'STORYBOARD', firstBoardAggregate.id, firstBoard.id, 'operator', '旧版本');
    const secondScript = await director.createManualScriptRevision(project.id, firstScript.id, { origin: 'MANUAL', title: '新脚本', titleCandidates: ['新脚本'], coverText: '新', topicKeywords: ['主题'], hook: '新开头', body: '新正文', createdBy: 'operator' }, 'operator');
    ...
    const secondBoard = await director.createStoryboardRevision(project.id, secondBoardAggregate.id, { origin: 'MANUAL', scriptRevisionId: secondScript.id, scenes: [{ sceneIndex: 1, voiceoverText: '新', durationHintSeconds: 2, visualInstruction: '新画面', assetKeywords: ['新'] }], createdBy: 'operator' });
    await director.approveStoryboard(project.id, secondBoard.id);
    await assert.rejects(() => approvals.create({ projectId: project.id, targetType: 'SCRIPT', targetId: scriptAggregate.id, targetRevisionId: secondScript.id, status: 'APPROVED', approver: 'operator' }), /PENDING/);
    await approvals.create({ projectId: project.id, targetType: 'SCRIPT', targetId: scriptAggregate.id, targetRevisionId: secondScript.id, status: 'PENDING', approver: 'operator' });
    await approvals.approve(project.id, 'SCRIPT', scriptAggregate.id, secondScript.id, 'operator');
    await approvals.create({ projectId: project.id, targetType: 'STORYBOARD', targetId: secondBoardAggregate.id, targetRevisionId: secondBoard.id, status: 'PENDING', approver: 'operator' });
    await approvals.approve(project.id, 'STORYBOARD', secondBoardAggregate.id, secondBoard.id, 'operator');
    ...
    await db.query('insert into assets (id, project_id, kind, checksum, byte_size, storage_key, lifecycle, metadata) values ($1, $2, $3, $4, $5, $6, $7, $8)', [renderAssetId, project.id, 'VIDEO_RENDER', 'sha256:v1-render', 100, 'renders/' + renderAssetId + '.mp4', 'READY', {}]);
    await db.query('insert into edit_manifests (id, project_id, revision, schema_version, manifest, status) values ($1, $2, $3, $4, $5, $6)', [manifestId, project.id, 1, 'EDIT_MANIFEST_V0', {}, 'PERSISTED']);
    await db.query('insert into renders (id, project_id, manifest_id, status, output_asset_id, finished_at) values ($1, $2, $3, $4, $5, now())', [renderId, project.id, manifestId, 'SUCCEEDED', renderAssetId]);
    await approvals.create({ projectId: project.id, targetType: 'RENDER', targetId: renderId, targetRevisionId: renderAssetId, status: 'PENDING', approver: 'operator' });
    const response = await app.inject({ method: 'GET', url: '/api/v1/projects/' + project.id + '/center' });
    assert.equal(response.statusCode, 200, response.body);
    assert.equal(response.json().stages[0].status, 'COMPLETE');
    assert.equal(response.json().stages[2].status, 'ACTION_REQUIRED');
    assert.notEqual(response.json().health.level, 'BLOCKED');
    assert.equal(response.json().actions.some((action: { id: string }) => action.id === 'approval-pending-RENDER'), true);
    assert.equal(response.json().actions.some((action: { id: string }) => action.id.includes('SCRIPT') || action.id.includes('STORYBOARD')), false);
  } finally {
    await app.close();
...
test('Project Center ignores Approval decisions for cancelled Publisher requests', async () => {
  ...
  try {
    ...
    await db.query('insert into assets (id, project_id, kind, checksum, byte_size, storage_key, lifecycle, metadata) values ($1, $2, $3, $4, $5, $6, $7, $8)', [assetId, project.id, 'VIDEO_RENDER', 'sha256:cancelled', 100, 'renders/' + assetId + '.mp4', 'READY', {}]);
    const account = await publisher.createAccount({ projectId: project.id, platformId: 'fake-platform', displayName: 'Cancelled Approval', credentialRef: 'server-ref', profileKey: 'server-profile', status: 'READY', capabilitySnapshot: { platformId: 'fake-platform', mediaTypes: ['video/mp4'], scheduling: false, requiresHumanConfirmation: false } });
    const request = await publisher.createRequest({ projectId: project.id, accountId: account.id, idempotencyKey: 'center-cancelled-' + randomUUID(), correlationId: 'center-cancelled-correlation-' + randomUUID(), revision: { assetId, assetChecksum: 'sha256:cancelled', title: '已取消', description: '', desiredPublishAt: null, createdBy: 'operator' } });
    await approvals.create({ projectId: project.id, targetType: 'PUBLISH', targetId: request.request.id, targetRevisionId: request.revision.id, status: 'PENDING', approver: 'operator' });
    ...
    assert.equal(response.json().actions.some((action: { id: string }) => action.id === 'approval-rejected'), false);
  } finally {
    await app.close();
    await cleanupProject(db, project.id);
    await db.end();
