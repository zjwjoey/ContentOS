...
test('Video Quick Edit UI exposes version history, trim/remove/reorder and exact render controls', async () => {
  ...
  for (const text of ['video/manifests', 'video/quick-edits', '生成 Quick Edit 版本', '创建精确渲染 Job', 'TRIM', 'REMOVE', 'REORDER', 'Manifest v', 'idempotencyKey']) assert.match(page, new RegExp(text.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
