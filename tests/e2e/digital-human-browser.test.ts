...
test('Digital Human V1 browser flow completes fake speech, avatar, preflight, blocked UI and Edit Manifest handoff', async () => {
  ...
  try {
    ...
    await page.getByText(/数字人：已就绪/, { exact: false }).waitFor({ state: 'visible', timeout: 20_000 });

    const createVoice = page.locator('details').filter({ hasText: '创建音色档案' });
    ...
    await page.getByText('已完成', { exact: true }).first().waitFor({ state: 'visible', timeout: 30_000 });

    const createAvatar = page.locator('details').filter({ hasText: '创建人物档案' });
    ...
    await page.getByText('人物档案已创建，请继续添加底片。', { exact: true }).waitFor({ state: 'visible', timeout: 20_000 });
    ...
    await avatarCard.getByRole('button', { name: '保存人物档案' }).click();
    await page.getByText('人物档案已保存。', { exact: true }).waitFor({ state: 'visible', timeout: 20_000 });
    ...
    await page.getByLabel('配音素材 ID').fill(speechGeneration.outputAssetId);
    ...
    await page.getByText('生成前检查：已通过', { exact: true }).waitFor({ state: 'visible', timeout: 20_000 });
    await page.getByLabel('配音素材 ID').fill('changed-after-preflight');
    await page.getByText('生成前检查：已通过', { exact: true }).waitFor({ state: 'hidden', timeout: 5_000 });
    await page.getByLabel('配音素材 ID').fill(speechGeneration.outputAssetId);
    ...
    await blockedPage.getByText(/数字人：不可用/, { exact: false }).waitFor({ state: 'visible', timeout: 20_000 });
    ...
    await blockedPage.getByLabel('配音素材 ID').fill(speechGeneration.outputAssetId);
