...
test('Quick Edit v2 renders the selected immutable timeline through the Video Worker', async () => {
  ...
  try {
    ...
    const version = await quickEdit.createVersion({
      projectId: project.id,
      parentManifestId: parentId,
      operations: [
        { type: 'REMOVE', clipIndex: 0 },
        { type: 'TRIM', clipIndex: 0, sourceInMs: 200, durationMs: 700 },
      ],
      createdBy: 'e2e',
    ...
    const invalidJob = await jobs.create({
      id: `job-invalid-${randomUUID()}`,
      projectId: project.id,
      type: 'VIDEO_RENDER',
      payload: { projectId: project.id, manifestId: version.id, manifestRevision: 999, manifestDigest: digest },
    ...
    const digestJob = await jobs.create({
      id: `job-digest-${randomUUID()}`,
      projectId: project.id,
      type: 'VIDEO_RENDER',
      payload: { projectId: project.id, manifestId: version.id, manifestRevision: 2, manifestDigest: digest },
      idempotencyKey: `digest-${randomUUID()}`,
      maxAttempts: 1,
    });
    ...
    worker = createVideoWorker({
      ...
      ffmpegPath,
      ffprobePath,
      workerId: `video-quick-edit-${randomUUID()}`,
      pollIntervalMs: 25,
    });
    await worker.start();
    ...
    const render = await db.query<{ manifest_id: string; status: string; output_asset_id: string }>(
      'select manifest_id, status, output_asset_id from renders where job_id = $1',
      [renderJob.id],
    );
    ...
    for (const jobId of [renderJobId, invalidJobId, digestJobId].filter((id): id is string => Boolean(id))) {
