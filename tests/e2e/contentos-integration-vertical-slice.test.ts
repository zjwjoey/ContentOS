...
class FlakyNetworkAdapter implements PublisherAdapter {
  ...
  capabilities(): PlatformCapabilityProfile {
    return { platformId: 'fake-platform', mediaTypes: ['video/mp4'], scheduling: false, requiresHumanConfirmation: false };
  }
  ...
  async publish(_context: PublisherContext, _snapshot: PublishSnapshot): Promise<PublishResult> {
    this.calls += 1;
    return this.calls === 1
      ? { status: 'FAILED', failure: { code: 'NETWORK_ERROR', classification: 'RETRYABLE', message: 'temporary network failure' } }
      : { status: 'PUBLISHED', externalPostId: 'fake-retry-post' };
  }
  async reconcile(_context: PublisherContext, _idempotencyKey: string): Promise<ExternalStateResult> {
...
async function publisherFixture(
  ...
  outcome: ConstructorParameters<typeof FakePublisherAdapter>[0] | PublisherAdapter = 'SUCCESS',
) {
  ...
  const account = await publisher.createAccount({
    ...
    displayName: 'Fake Integration',
    credentialRef: 'fake-credential:integration',
  ...
  const request = await publisher.createRequest({
    ...
    idempotencyKey: `publisher-integration-${randomUUID()}`,
    correlationId: `correlation-${randomUUID()}`,
    revision: {
      assetId,
      assetChecksum: checksum,
      title: '集成验收视频',
      description: 'ContentOS 集成验收',
  ...
  const job = await jobs.create({
    ...
    projectId: project.id,
    payload,
    idempotencyKey: `job-publisher-integration-${randomUUID()}`,
    maxAttempts: 3,
  });
...
test('ContentOS public services compose Director, Video, Approval, Publisher and Project status', async () => {
  ...
  try {
    ...
    const storyboardJob = await directorJobs.createStoryboardGeneration({
      ...
      correlationId: `storyboard-${randomUUID()}`,
    });
    ...
    const videoJob = await new DirectorVideoService(director, video).createVideoJob(project.id, {
      videoAssetIds,
      sceneAssetBindings,
      voiceAssetId: voice.id,
      targetDurationMs: 3_000,
      subtitleText: 'ContentOS Integration',
      seed: 7,
    });
    const renderResult = await new JobRunner(jobs, 'integration-video-worker').run(
      videoJob.id,
    ...
    await renderApproval.create({
      projectId: project.id,
      targetType: 'RENDER',
      targetId: rendered.renderId,
      targetRevisionId: rendered.outputAssetId,
      status: 'PENDING',
      approver: 'operator',
    });
    ...
    const account = await publisher.createAccount({
      ...
      credentialRef: 'fake-credential:full-flow',
      profileKey: `profile-${randomUUID()}`,
      status: 'READY',
      capabilitySnapshot: { platformId: 'fake-platform', mediaTypes: ['video/mp4'], scheduling: false, requiresHumanConfirmation: false },
    });
    const request = await publisher.createRequest({
      projectId: project.id,
      accountId: account.id,
      idempotencyKey: `full-publish-${randomUUID()}`,
      correlationId: `full-correlation-${randomUUID()}`,
      revision: {
        assetId: rendered.outputAssetId,
        assetChecksum: output.rows[0]!.checksum,
        title: 'ContentOS 全链路',
        description: 'Director 到 Publisher',
        desiredPublishAt: null,
        createdBy: 'operator',
      },
    });
    const publishApproval = new ApprovalService(db);
    await publishApproval.create({
      projectId: project.id,
      targetType: 'PUBLISH',
    ...
    const publishJob = await jobs.create({
      id: publishJobId,
      type: 'PUBLISH',
      projectId: project.id,
      payload,
      idempotencyKey: `full-publish-job-${randomUUID()}`,
      maxAttempts: 3,
    });
    ...
    try {
      const worker = createPublisherWorker({
        ...
        fakePublisher: new FakePublisherService(fakeRoot),
        workerId: 'full-integration-publisher',
      });
      await worker.start();
    ...
    assert.equal(
      (
        await probeMedia(
          storage.objectPath(
            (await db.query<{ storage_key: string }>('select storage_key from assets where id = $1', [rendered.outputAssetId])).rows[0]!.storage_key,
          ),
          ffprobePath,
        )
      ).format,
...
test('ContentOS Publisher network failure retries and then succeeds', async () => {
  ...
  let root = '';
  try {
    const data = await publisherFixture(db, new FlakyNetworkAdapter());
    ...
    const worker = createPublisherWorker({
      ...
      projects: new ProjectService(db),
      assets: new AssetCatalogService(db),
      fakePublisher: data.fake,
      workerId: 'integration-network',
    });
    await worker.start();
    const first = (await worker.execute('PUBLISH', { jobId: data.job.id })) as { state: string };
    assert.equal(first.state, 'RETRY_WAIT');
    assert.equal((await data.publisher.getRequest(data.request.request.id))?.status, 'FAILED');
    await data.jobs.requeue(data.job.id);
    const second = (await worker.execute('PUBLISH', { jobId: data.job.id })) as { state: string };
    assert.equal(second.state, 'SUCCEEDED');
    assert.equal((await data.publisher.getRequest(data.request.request.id))?.status, 'PUBLISHED');
    await worker.shutdown('integration');
  } finally {
    if (projectId) await cleanupProject(db, projectId);
    await db.end();
  }
});
...
test('ContentOS Publisher auth expiry is human action and does not auto-retry', async () => {
  ...
  let projectId = '';
  try {
    const data = await publisherFixture(db, 'AUTH_EXPIRED');
    projectId = data.project.id;
    const worker = createPublisherWorker({
      service: data.publisher,
      jobs: data.jobs,
    ...
    assert.equal(aggregate?.request.status, 'FAILED');
    assert.equal(aggregate?.nextAction, 'NEEDS_HUMAN_ACTION');
    assert.equal((await data.jobs.get(data.job.id))?.state, 'FAILED');
    await worker.shutdown('integration');
  } finally {
    if (projectId) await cleanupProject(db, projectId);
    await db.end();
...
test('ContentOS Publisher unknown side effect reconciles to one external post', async () => {
  ...
  let projectId = '';
  try {
    const data = await publisherFixture(db, 'BROWSER_CRASH');
    projectId = data.project.id;
    const worker = createPublisherWorker({
      service: data.publisher,
      jobs: data.jobs,
      ...
      assets: new AssetCatalogService(db),
      fakePublisher: data.fake,
      workerId: 'integration-unknown',
    });
    await worker.start();
    const first = (await worker.execute('PUBLISH', { jobId: data.job.id })) as { state: string };
    assert.equal(first.state, 'SUCCEEDED');
    assert.equal((await data.publisher.getRequest(data.request.request.id))?.status, 'RECONCILING');
    const reconcileJob = await data.jobs.getByIdempotencyKey(
      `publisher:reconcile:${data.request.request.id}:${(data.job.payload as { revisionId: string }).revisionId}`,
    );
    assert.ok(reconcileJob);
    const second = (await worker.execute('PUBLISH_RECONCILE', { jobId: reconcileJob!.id })) as { state: string };
    assert.equal(second.state, 'SUCCEEDED');
    assert.equal((await data.publisher.getRequest(data.request.request.id))?.status, 'PUBLISHED');
    assert.equal((await db.query('select id from publisher_external_posts where request_id = $1', [data.request.request.id])).rowCount, 1);
    await worker.shutdown('integration');
  } finally {
    if (projectId) await cleanupProject(db, projectId);
    await db.end();
