...
export default function DigitalHumanIndexPage() {
  ...
  const generateAvatar = async () => {
    ...
    setBusy(true); setNotice('');
    try {
      const preflight = await readJson<{ status: string; checks: Array<{ status: string; message: string }> }>(await fetch(`/api/v1/projects/${projectId}/digital-human/avatar-generations/preflight`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ avatarProfileId: avatarId, avatarClipId: clipId, speechAssetId }) }));
      if (preflight.status !== 'READY') throw new Error(preflight.checks.find((item) => item.status === 'BLOCKED')?.message || '生成前检查未通过');
      await readJson(await fetch(`/api/v1/projects/${projectId}/digital-human/avatar-generations`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ avatarProfileId: avatarId, avatarClipId: clipId, speechAssetId }) }));
  ...
  return <main className="shell">
    ...
    {loading ? <section className="card"><p className="muted">正在准备数字人工作台…</p></section> : !projectId ? <section className="card form-error"><h2>还没有可用工作空间</h2><p>请先在首页创建一个内容项目，数字人工作台会自动接入，不再要求你选择项目。</p><a className="module-nav-link" href="/">返回首页</a></section> : <>
      ...
      <section className="grid grid-2">
        ...
        <section className="card"><div className="section-title"><h2>数字人视频</h2><span>可交互预览</span></div><label>人物<select value={avatarId} onChange={(event) => setAvatarId(event.target.value)}><option value="">请选择人物</option>{avatars.map((avatar) => <option key={avatar.id} value={avatar.id}>{avatar.name} · {label(avatar.status)}</option>)}</select></label><label>人物底片<select value={clipId} onChange={(event) => setClipId(event.target.value)}><option value="">请选择底片</option>{clips.map((clip) => <option key={clip.id} value={clip.id}>{clip.name} · 使用 {clip.usageCount} 次</option>)}</select></label><label>配音素材 ID<input value={speechAssetId} onChange={(event) => setSpeechAssetId(event.target.value)} placeholder="生成配音后自动填入" /></label><button type="button" disabled={busy || !avatarId || !clipId || !speechAssetId} onClick={() => void generateAvatar()}>生成数字人视频</button><p className="muted">先生成配音，再点击生成数字人视频。生成前会自动检查素材、时长和服务状态。</p></section>
      </section>
