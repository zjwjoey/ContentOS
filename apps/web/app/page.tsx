...
export default function HomePage() {
  ...
  const createProject = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      router.push(`/projects/${project.id}/director`);
    } catch (error) { setMessage(error instanceof Error ? error.message : '项目创建失败。'); }
    finally { setCreating(false); }
  };

  return <main className="shell">
    <header><p className="eyebrow">ContentOS / Operator</p><h1>内容项目</h1><p className="muted">从项目进入 Director，追踪 Brief、Job、Script 和 Storyboard 的版本状态。</p></header>
    <section className="card">
      ...
      <form className="project-create" onSubmit={createProject}>
        ...
        <button type="submit" disabled={creating}>{creating ? '创建中…' : '创建并进入 Director'}</button>
      </form>
    </section>
    <section className="card">
      <div className="section-title"><h2>项目列表</h2><span>{loading ? '加载中…' : `${projects.length} 个项目`}</span></div>
      {message && <p className="form-error">{message}</p>}
      {loading ? <p className="muted">正在读取项目…</p> : projects.length === 0 ? <p className="muted">暂无项目，请先创建一个项目。</p> : <ul className="project-list">{projects.map((project) => <li key={project.id}><Link href={`/projects/${project.id}/director`}><span>{project.name || project.id}</span><small>{project.status}</small></Link></li>)}</ul>}
    </section>
  </main>;
}
