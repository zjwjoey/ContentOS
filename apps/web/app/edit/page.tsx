...
function statusLabel(status: string, item?: { succeededCount: number; failedCount: number }): string {
  if (status === 'SUCCEEDED') return '已完成';
  if (status === 'PARTIAL') return `${item?.succeededCount || 0} 成功 / ${item?.failedCount || 0} 失败`;
  if (status === 'FAILED') return '剪辑失败';
  if (status === 'RUNNING') return '正在剪辑';
...
async function RecentEdits() {
  ...
  return <ul className="project-list">{items.map((item) => <li key={item.id}><Link href={`/edit/history?batch=${encodeURIComponent(item.id)}`}><span><strong>{item.title}</strong><small>{item.mode === 'MIX' ? '批量混剪' : '脚本剪辑'} · {item.totalCount} 条</small></span><small>{statusLabel(item.status, item)}</small></Link></li>)}</ul>;
}
