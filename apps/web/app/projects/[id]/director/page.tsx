...
export default function DirectorPage({ params }: { params: { id: string } }) {
  ...
  return <main className="shell"><header><p className="eyebrow">Project / {projectId}</p><h1>Director 工作台</h1><p className="muted">Brief 是输入事实，Script 和 Storyboard 以追加版本保存；生成工作通过 Durable Job 执行。</p><nav className="module-nav"><Link href={`/projects/${projectId}/publisher`}>进入 Publisher</Link></nav></header>
    ...
    <section className="card"><div className="section-title"><h2>Storyboard revisions</h2><span>{storyboards.length} 个版本</span></div><ul className="revision-list">{storyboards.map((storyboard) => <li key={storyboard.id}><strong>V{storyboard.revision} · {storyboard.status}</strong><span>绑定 Script {storyboard.scriptRevisionId}</span><small>{storyboard.scenes.map((scene) => `${scene.sceneIndex}. ${scene.voiceoverText}`).join(' / ')}</small>{storyboard.status === 'DRAFT' && <button type="button" onClick={() => void approveStoryboard(storyboard.id)}>批准 Storyboard</button>}</li>)}</ul></section>
  </main>;
}
