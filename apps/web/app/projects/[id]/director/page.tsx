...
export default function DirectorPage({ params }: { params: { id: string } }) {
  ...
  const createBrief = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const created = await response.json() as Brief; setBrief(created); setJob(null); setMessage(`Brief 版本 ${created.revision} 已创建。`);
  };
  ...
  const generateScript = async () => {
    ...
    setJob(response.ok ? data : null); setMessage(response.ok ? `脚本任务 ${data.jobId} 已入队，正在等待 Worker。` : data.error?.message || '任务创建失败。');
  };
  ...
  return <main className="shell"><header><p className="eyebrow">Project / {projectId}</p><h1>Director 工作台</h1><p className="muted">Brief 是输入事实，Script 和 Storyboard 以追加版本保存；生成工作通过 Durable Job 执行。</p><nav className="module-nav"><Link href={`/projects/${projectId}/assets`}>返回 Assets</Link>{videoPrerequisiteReady && <Link href={`/projects/${projectId}/video`}>进入 Video</Link>}<Link href={`/projects/${projectId}/publisher`}>进入 Publisher</Link></nav></header>
    <section className="grid"><form className="card" onSubmit={createBrief}><div className="section-title"><h2>ContentBrief</h2><span>{brief ? `当前版本 ${brief.revision}` : '未创建'}</span></div>
    ...
    </form><section className="card"><div className="section-title"><h2>生成任务</h2><span>{job?.state || '—'}</span></div><p className="muted">API 只入队，Worker 执行 Fake AI；页面会自动读取状态。</p><button type="button" onClick={() => void generateScript()} disabled={!brief || Boolean(job && !terminalStates.has(job.state))}>生成脚本</button>{job && <p className="status">{job.jobId || job.id} · {job.state} · 尝试 {job.attemptCount ?? 0}/{job.maxAttempts ?? '—'}</p>}{message && <p className="status">{message}</p>}</section></section>
    <section className="card"><div className="section-title"><h2>脚本版本</h2><span>{scripts.length} 个版本</span></div><ul className="revision-list">{scripts.map((script) => <li key={script.id}><strong>版本 {script.revision} · {script.title}</strong><span>{script.status} / {script.origin}</span><small>Hook：{script.hook}<br />Body：{script.body}<br />CTA：{script.cta || '—'}</small><label>手动编辑 Body（保存会新建版本）<textarea value={scriptDrafts[script.id] ?? script.body} onChange={(event) => setScriptDrafts({ ...scriptDrafts, [script.id]: event.target.value })} /></label><button type="button" onClick={() => void saveScriptEdit(script)}>保存为新版本</button>{script.status === 'DRAFT' && <><button type="button" onClick={() => void requestApproval('SCRIPT', script.id)}>提交脚本审批</button><button type="button" onClick={() => void acceptScript(script.id)}>接受脚本（兼容快捷操作）</button><button type="button" onClick={() => void reviseScript(script)}>生成手工版本 {script.revision + 1}</button></>}{script.status === 'ACCEPTED' && <button type="button" onClick={() => void generateStoryboard(script.id)}>生成分镜任务</button>}</li>)}</ul></section>
    <section className="card"><div className="section-title"><h2>分镜版本 · 画面指令 · 素材关键词</h2><span>{storyboards.length} 个版本</span></div><ul className="revision-list">{storyboards.map((storyboard) => <li key={storyboard.id}><strong>版本 {storyboard.revision} · {storyboard.status}</strong><span>绑定脚本 {storyboard.scriptRevisionId}</span><small>{storyboard.scenes.map((scene) => `场景 ${String(scene.sceneIndex).padStart(2, '0')} · ${scene.durationHintSeconds.toFixed(1)} 秒 · 口播：${scene.voiceoverText} · 画面：${scene.visualInstruction} · 素材关键词：${scene.assetKeywords.join(' / ')}`).join(' / ')}</small><label>手动修改场景 JSON（保存会新建版本）<textarea value={storyboardDrafts[storyboard.id] ?? JSON.stringify(storyboard.scenes, null, 2)} onChange={(event) => setStoryboardDrafts({ ...storyboardDrafts, [storyboard.id]: event.target.value })} /></label><button type="button" onClick={() => void saveStoryboardEdit(storyboard)}>保存为新版本</button>{storyboard.status === 'DRAFT' && <><button type="button" onClick={() => void requestApproval('STORYBOARD', storyboard.id)}>提交分镜审批</button><button type="button" onClick={() => void approveStoryboard(storyboard.id)}>批准分镜（兼容快捷操作）</button></>}</li>)}</ul></section>
