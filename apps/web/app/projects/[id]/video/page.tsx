...
export default function VideoPage({ params }: { params: { id: string } }) {
  ...
  return <main className="shell"><header><p className="eyebrow">Project / {projectId}</p><h1>Video 工作台</h1><p className="muted">只使用已批准 Director pair 和 READY 素材；渲染通过 Durable Job 执行。</p><nav className="module-nav"><Link href={`/projects/${projectId}/assets`}>Assets</Link><Link href={`/projects/${projectId}/director`}>Director</Link><Link href={`/projects/${projectId}/approvals`}>Approval Gate</Link></nav></header>
    ...
    <section className="card"><div className="section-title"><h2>Quick Edit Manifest 版本</h2><span>{manifests.length} 个版本</span></div><div className="module-nav">{manifests.map((manifest) => <button type="button" key={manifest.id} onClick={() => void chooseManifest(manifest.id)} disabled={busy}>v{manifest.revision} · {manifest.status}</button>)}</div>{activeManifest ? <><p className="muted">当前编辑目标：Manifest v{activeManifest.revision} · {activeManifest.manifest.timeline.length} 个镜头</p><div className="revision-list">{activeManifest.manifest.timeline.map((clip, index) => <div key={`${activeManifest.id}-${index}`} className="checkbox-row"><span>镜头 {index + 1} · {clip.assetId} · {clip.durationMs}ms</span><button type="button" onClick={() => { setTrimClipIndex(index); setTrimStart(clip.sourceInMs); setTrimDuration(clip.durationMs); }}>选中裁剪</button><button type="button" onClick={() => addRemove(index)}>移除</button><button type="button" onClick={() => addMove(index, -1)} disabled={pendingOperations.length > 0}>上移</button><button type="button" onClick={() => addMove(index, 1)} disabled={pendingOperations.length > 0}>下移</button></div>)}</div><label>裁剪镜头索引<input type="number" min={0} value={trimClipIndex} onChange={(event) => setTrimClipIndex(Number(event.target.value))} disabled={busy} /></label><label>起始毫秒<input type="number" min={0} value={trimStart} onChange={(event) => setTrimStart(Number(event.target.value))} disabled={busy} /></label><label>时长毫秒<input type="number" min={1} value={trimDuration} onChange={(event) => setTrimDuration(Number(event.target.value))} disabled={busy} /></label><button type="button" onClick={addTrim} disabled={busy}>加入裁剪操作</button>{pendingOperations.length > 0 && <p className="status">待提交操作：{pendingOperations.length} 个（按顺序应用）</p>}<button type="button" onClick={() => void createQuickEdit()} disabled={busy || pendingOperations.length === 0}>生成 Quick Edit 版本</button><button type="button" onClick={() => void renderManifest()} disabled={busy || pendingOperations.length > 0}>创建精确渲染 Job</button></> : <p className="muted">先创建一次渲染 Job，系统会生成可编辑的 Manifest 版本。</p>}</section><section className="card"><div className="section-title"><h2>Render 历史</h2><span>{snapshot?.renderHistory.length || 0} 条</span></div><ul className="revision-list">{snapshot?.renderHistory.map((render) => <li key={render.renderId}><strong>{render.renderId}</strong><span>{render.status}</span>{render.outputAssetId && <small>Output Asset {render.outputAssetId}</small>}</li>)}</ul></section>
  </main>;
}
