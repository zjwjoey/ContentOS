...
export default function PublisherPage({ params }: { params: { id: string } }) {
  ...
  return <main className="shell">
    ...
    <section className="grid">
      ...
      <form className="card" onSubmit={createRequest}><div className="section-title"><h2>项目发布交接</h2><span>{selectedAccountIds.length} 个账号</span></div><fieldset><legend>目标账号</legend>{accounts.map((account) => <label key={account.id}><input type="checkbox" checked={selectedAccountIds.includes(account.id)} onChange={(event) => setSelectedAccountIds((current) => event.target.checked ? [...current, account.id] : current.filter((id) => id !== account.id))} />{account.displayName} · {account.platformId} · {account.status}</label>)}</fieldset><label>成片 Render Asset<select value={assetId} onChange={(event) => setAssetId(event.target.value)} required><option value="">请选择可发布成片</option>{assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.kind} · {asset.byteSize} bytes</option>)}</select></label><label>标题<input value={title} onChange={(event) => setTitle(event.target.value)} required maxLength={200} /></label><label>描述<textarea value={description} onChange={(event) => setDescription(event.target.value)} /></label><label>Hashtags（逗号分隔）<input value={hashtags} onChange={(event) => setHashtags(event.target.value)} placeholder="效率工具, 内容生产" /></label><label>封面 Asset ID（可选）<input value={coverAssetId} onChange={(event) => setCoverAssetId(event.target.value)} placeholder="可选的项目封面素材 ID" /></label><button disabled={busy || !selectedAccountIds.length || !assetId}>创建项目发布草稿</button></form>
    </section>
