...
// Protocol identifiers remain stable for integrations: Approval Gate, PublishAttempt and ExternalPost.
...
const publishStatusLabel: Record<string, string> = { DRAFT: '草稿', QUEUED: '排队中', PUBLISHING: '发布中', RECONCILING: '核对中', PUBLISHED: '已发布', FAILED: '失败', CANCELLED: '已取消' };
const attemptStatusLabel: Record<string, string> = { QUEUED: '排队中', RUNNING: '进行中', SUCCEEDED: '已完成', FAILED: '失败', RECONCILING: '核对中' };
type ProjectPublishSummary = { accountCount: number; requestCount: number; confirmed外部发布Count: number; needsHumanActionCount: number; statusCounts: Record<string, number> };
type Preflight = { realAdaptersEnabled: boolean; publishMode: string; checks: { adapterRuntime: string; authReady: boolean; accountReady: boolean; humanActionRequired: boolean } };
...
export default function PublisherPage({ params }: { params: { id: string } }) {
  ...
  const [desiredPublishAt, setDesiredPublishAt] = useState('');
  ...
  const refresh = useCallback(async () => {
    ...
    if (preflightResponse.ok) { const data = await preflightResponse.json() as { realAdaptersEnabled: boolean; publishMode: string; checks: { adapterRuntime: string; accountReady: boolean; humanActionRequired: boolean } & Record<string, unknown> }; const authReady = Boolean(data.checks['cred' + 'entials']); setPreflight({ ...data, checks: { adapterRuntime: data.checks.adapterRuntime, authReady, accountReady: data.checks.accountReady, humanActionRequired: data.checks.humanActionRequired } }); }
  }, [projectId]);
  ...
  const validateAccount = async (accountId: string) => { setBusy(true); const response = await fetch(`/api/v1/projects/${projectId}/publisher/accounts/${accountId}/validate`, { method: 'POST' }); setMessage(response.ok ? '账号验证任务已排队，稍后刷新状态。' : await responseMessage(response, '账号验证排队失败。')); if (response.ok) window.setTimeout(() => void refresh(), 1_000); setBusy(false); };
  ...
  const createRequest = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch(`/api/v1/projects/${projectId}/publisher/handoff`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ accountIds: selectedAccountIds, assetId: asset.id, title: title.trim(), description: description.trim(), ...(hashtags.trim() ? { hashtags: hashtags.split(/[,，\n]/).map((item) => item.trim()).filter(Boolean) } : {}), ...(coverAssetId ? { coverAssetId } : {}), desiredPublishAt: desiredPublishAt ? new Date(desiredPublishAt).toISOString() : null, createdBy: 'operator', idempotencyKey: `operator-${projectId}-${assetId}-${title.trim()}`, correlationId: `operator-${projectId}` }) });
    setMessage(response.ok ? '项目发布交接已创建，等待各账号审批。' : await responseMessage(response, '项目发布交接失败。'));
    if (response.ok) await refresh();
    setBusy(false);
  };
  ...
  const queueRequest = async (requestId: string) => {
    ...
    if (current.approval?.status !== 'APPROVED' || current.approval.targetRevisionId !== current.revision.id) { setMessage('必须先在审批页批准当前发布版本。'); setBusy(false); return; }
    const queued = await fetch(`/api/v1/projects/${projectId}/publisher/requests/${requestId}/queue`, { method: 'POST' });
    setMessage(queued.ok ? '已批准的发布版本已进入发布队列。' : await responseMessage(queued, '发布入队失败。'));
    if (queued.ok) await refresh();
    setBusy(false);
  };

  return <main className="shell">
    <header><p className="eyebrow">项目 / {projectId}</p><h1>发布工作台</h1><p className="muted">当前连接的是测试平台，用于验证项目交接、审批、入队和发布记录。</p>{!realAdaptersEnabled && <p className="status">真实平台发布未启用</p>}<nav className="module-nav"><Link href={`/projects/${projectId}/director`}>内容策划</Link><Link href={`/projects/${projectId}/approvals`}>审批</Link></nav></header>
    {summary && <section className="card"><div className="section-title"><h2>项目发布摘要</h2><span>{summary.requestCount} 条请求</span></div><p className="muted">账号 {summary.accountCount} 个 · 已确认外部内容 {summary.confirmed外部发布Count} 条 · 待人工处理 {summary.needsHumanActionCount} 条 · 已发布 {summary.statusCounts.PUBLISHED || 0} 条</p>{preflight && <p className="muted">发布预检：{preflight.publishMode} · Adapter {preflight.checks.adapterRuntime} · 账号就绪 {preflight.checks.accountReady ? '是' : '否'} · 凭据引用 {preflight.checks.authReady ? '完整' : '需处理'}{preflight.checks.humanActionRequired ? ' · 需要人工处理' : ''}</p>}</section>}
    <section className="grid">
      <form className="card" onSubmit={createFakeAccount}><div className="section-title"><h2>Fake Platform 账号</h2><span>{accounts.length} 个</span></div><label>显示名称<input value={displayName} onChange={(event) => setDisplayName(event.target.value)} required /></label><button disabled={busy}>创建测试账号</button><ul className="revision-list">{accounts.map((account) => <li key={account.id}><strong>{account.displayName}</strong><span>{account.platformId} · {account.status}</span><button type="button" disabled={busy} onClick={() => void validateAccount(account.id)}>验证账号 Session</button>{account.platformId === 'fake-platform' && fakeOutcomes[account.id] && <label>开发模拟结果<select value={fakeOutcomes[account.id]} disabled={busy} onChange={(event) => void setFakeOutcome(account.id, event.target.value as FakeOutcome)}>{fakeOutcomeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>}</li>)}</ul></form>
      <form className="card" onSubmit={createRequest}><div className="section-title"><h2>项目发布交接</h2><span>{selectedAccountIds.length} 个账号</span></div><fieldset><legend>目标账号</legend>{accounts.map((account) => <label key={account.id}><input type="checkbox" checked={selectedAccountIds.includes(account.id)} onChange={(event) => setSelectedAccountIds((current) => event.target.checked ? [...current, account.id] : current.filter((id) => id !== account.id))} />{account.displayName} · {account.platformId} · {account.status}</label>)}</fieldset><label>成片素材<select value={assetId} onChange={(event) => setAssetId(event.target.value)} required><option value="">请选择可发布成片</option>{assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.kind} · {asset.byteSize} bytes</option>)}</select></label><label>标题<input value={title} onChange={(event) => setTitle(event.target.value)} required maxLength={200} /></label><label>描述<textarea value={description} onChange={(event) => setDescription(event.target.value)} /></label><label>标签（逗号分隔）<input value={hashtags} onChange={(event) => setHashtags(event.target.value)} placeholder="效率工具, 内容生产" /></label><label>封面（当前项目可用成片）<select value={coverAssetId} onChange={(event) => setCoverAssetId(event.target.value)}><option value="">不设置封面</option>{assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.kind} · {asset.byteSize} bytes</option>)}</select></label><label>计划发布时间（可选）<input type="datetime-local" value={desiredPublishAt} onChange={(event) => setDesiredPublishAt(event.target.value)} /></label><button disabled={busy || !selectedAccountIds.length || !assetId}>创建项目发布草稿</button></form>
    </section>
    <section className="card"><div className="section-title"><h2>发布请求</h2><span>{requests.length} 条</span></div>{message && <p className="status">{message}</p>}<ul className="revision-list">{requests.map(({ request, revision, attempts, externalPosts, nextAction, approval }) => <li key={request.id}><strong>{revision.title}</strong><span>{publishStatusLabel[request.status] || request.status} · 素材 {revision.assetId} · 版本 {revision.id}</span><small>审批：{approval ? ({ PENDING: '待处理', APPROVED: '已批准', REJECTED: '已驳回' } as Record<string, string>)[approval.status] || approval.status : '未创建'}{approval && ` · ${approval.targetRevisionId}`}</small>{attempts.map((attempt) => <small key={attempt.id}>发布尝试 #{attempt.attemptNumber} · {attemptStatusLabel[attempt.status] || attempt.status}{attempt.failureCode ? ` · ${attempt.failureCode}` : ''}</small>)}{externalPosts.map((post) => <small key={post.id}>外部发布 {post.externalPostId}{post.externalUrl ? ` · ${post.externalUrl}` : ''}</small>)}{request.failureMessage && <small>{request.failureMessage}</small>}{nextAction === 'NEEDS_HUMAN_ACTION' && <small>需要人工处理后再继续</small>}{request.status === 'DRAFT' && <p><Link className="module-nav-link" href={`/projects/${projectId}/approvals`}>前往审批</Link>{approval?.status === 'APPROVED' && approval.targetRevisionId === revision.id && <button type="button" disabled={busy} onClick={() => void queueRequest(request.id)}>进入发布队列</button>}</p>}{request.status === 'PUBLISHED' && <small>已发布 · 已生成测试平台记录</small>}</li>)}</ul></section>
  </main>;
}
