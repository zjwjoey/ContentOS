...
type RequestAggregate = {
  ...
  revision: { id: string; title: string; description: string; assetId: string };
  attempts: Array<{
    id: string;
  ...
  externalPosts: Array<{ id: string; externalPostId: string; externalUrl: string | null }>;
  nextAction: 'NEEDS_HUMAN_ACTION' | null;
  approval?: { status: 'PENDING' | 'APPROVED' | 'REJECTED'; targetRevisionId: string };
};
type ProjectPublishSummary = {
  accountCount: number;
  requestCount: number;
...
const fakeOutcomeOptions: Array<{ value: FakeOutcome; label: string }> = [
  ...
  { value: 'NETWORK', label: '网络故障（可重试）' },
  { value: 'AUTH_EXPIRED', label: '登录失效（人工处理）' },
  { value: 'VERIFICATION', label: '需要验证（人工处理）' },
  { value: 'BROWSER_CRASH', label: '浏览器崩溃（未知状态）' },
  { value: 'UNKNOWN_SIDE_EFFECT', label: '未知状态（可能已发布）' },
  { value: 'UNKNOWN_NO_SIDE_EFFECT', label: '未知状态（未确认发布）' },
  { value: 'RATE_LIMIT', label: '频率限制（可重试）' },
  { value: 'DOM_DRIFT', label: '平台结构变化（失败）' },
];
...
export default function PublisherPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (accountResponse.ok) {
      ...
      setSelectedAccountIds((current) =>
        current.length ? current.filter((id) => nextAccounts.some((account) => account.id === id)) : nextAccounts[0]?.id ? [nextAccounts[0].id] : [],
      );
      const simulated = await Promise.all(
        nextAccounts
          .filter((account) => account.platformId === 'fake-platform')
          .map(async (account) => {
    ...
    if (requestResponse.ok) {
      ...
      const approvalItems = approvalResponse.ok
        ? (
            (await approvalResponse.json()) as {
              items: Array<{ targetType: string; targetId: string; targetRevisionId: string; status: 'PENDING' | 'APPROVED' | 'REJECTED' }>;
            }
          ).items
        : [];
      const aggregates = await Promise.all(
        items.map(async (item) => {
          ...
          if (!response.ok)
            return { request: item, revision: { id: '', title: item.id, description: '', assetId: '' }, attempts: [], externalPosts: [], nextAction: null };
          const aggregate = (await response.json()) as RequestAggregate;
          const approval = approvalItems.find(
            (candidate) => candidate.targetType === 'PUBLISH' && candidate.targetId === item.id && candidate.targetRevisionId === aggregate.revision.id,
          );
  ...
  const createFakeAccount = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true);
    setMessage('');
    const response = await fetch(`/api/v1/projects/${projectId}/publisher/accounts`, {
      method: 'POST',
  ...
  const setFakeOutcome = async (accountId: string, outcome: FakeOutcome) => {
    setBusy(true);
    setMessage('');
    const response = await fetch(`/api/v1/projects/${projectId}/publisher/accounts/${accountId}/fake-outcome`, {
      method: 'PUT',
      headers: { 'content-type': 'application/json' },
    ...
    if (!response.ok) setMessage(await responseMessage(response, '开发模拟结果更新失败。'));
      ...
      setFakeOutcomes((current) => ({ ...current, [accountId]: outcome }));
      setMessage('开发模拟结果已更新，仅影响 Fake Platform。');
    }
    setBusy(false);
  };
  ...
  const createRequest = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!asset) {
      setMessage('请选择当前项目的可发布成片。');
      setBusy(false);
      return;
    }
    if (!selectedAccountIds.length) {
      setMessage('至少选择一个发布账号。');
      setBusy(false);
      return;
    }
    const response = await fetch(`/api/v1/projects/${projectId}/publisher/handoff`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        ...
        createdBy: 'operator',
        idempotencyKey: `operator-${projectId}-${assetId}-${title.trim()}`,
        correlationId: `operator-${projectId}`,
      }),
    });
  ...
  const queueRequest = async (requestId: string) => {
    ...
    if (!current?.revision.id) {
      setMessage('发布 Revision 信息缺失。');
      setBusy(false);
      return;
    }
    if (current.approval?.status !== 'APPROVED' || current.approval.targetRevisionId !== current.revision.id) {
      setMessage('必须先在 Approval Gate 批准当前发布 Revision。');
      setBusy(false);
      return;
    }
  ...
  return (
    <main className="shell">
      <header>
        ...
        <h1>Publisher 工作台</h1>
        <p className="muted">当前只连接 Fake Platform，用于验证项目交接、Approval Gate、入队、Worker 执行和发布记录。</p>
      ...
      {summary && (
        <section className="card">
          <div className="section-title">
            <h2>项目发布摘要</h2>
            <span>{summary.requestCount} 条请求</span>
          </div>
          <p className="muted">
            账号 {summary.accountCount} 个 · 已确认外部内容 {summary.confirmedExternalPostCount} 条 · 待人工处理 {summary.needsHumanActionCount} 条 · 已发布{' '}
            {summary.statusCounts.PUBLISHED || 0} 条
          </p>
        </section>
      )}
      <section className="grid">
        <form className="card" onSubmit={createFakeAccount}>
          <div className="section-title">
            <h2>Fake Platform 账号</h2>
          ...
          <button disabled={busy}>创建测试账号</button>
          <ul className="revision-list">
            {accounts.map((account) => (
              <li key={account.id}>
                <strong>{account.displayName}</strong>
                <span>
                  {account.platformId} · {account.status}
                </span>
                {account.platformId === 'fake-platform' && fakeOutcomes[account.id] && (
                  <label>
                    开发模拟结果
                    <select
                      value={fakeOutcomes[account.id]}
                      disabled={busy}
                      onChange={(event) => void setFakeOutcome(account.id, event.target.value as FakeOutcome)}
                    >
                      {fakeOutcomeOptions.map((option) => (
        ...
        <form className="card" onSubmit={createRequest}>
          <div className="section-title">
            <h2>项目发布交接</h2>
            <span>{selectedAccountIds.length} 个账号</span>
          </div>
          <fieldset>
            <legend>目标账号</legend>
            {accounts.map((account) => (
              <label key={account.id}>
                <input
                  ...
                  onChange={(event) =>
                    setSelectedAccountIds((current) => (event.target.checked ? [...current, account.id] : current.filter((id) => id !== account.id)))
                  }
                />
                {account.displayName} · {account.platformId} · {account.status}
              </label>
            ))}
          </fieldset>
          <label>
            ...
            <select value={assetId} onChange={(event) => setAssetId(event.target.value)} required>
              <option value="">请选择可发布成片</option>
              {assets.map((asset) => (
                <option key={asset.id} value={asset.id}>
                  {asset.id} · {asset.byteSize} bytes
                </option>
              ))}
            </select>
          </label>
          ...
          <button disabled={busy || !selectedAccountIds.length || !assetId}>创建项目发布草稿</button>
        </form>
      </section>
      <section className="card">
        <div className="section-title">
        ...
        <ul className="revision-list">
          {requests.map(({ request, revision, attempts, externalPosts, nextAction, approval }) => (
            <li key={request.id}>
              <strong>{revision.title}</strong>
              <span>
                {request.status} · Asset {revision.assetId} · Revision {revision.id}
              </span>
              <small>
                Approval：{approval?.status || '未创建'}
                {approval && ` · ${approval.targetRevisionId}`}
              </small>
              {attempts.map((attempt) => (
                <small key={attempt.id}>
                  PublishAttempt #{attempt.attemptNumber} · {attempt.status}
                  {attempt.failureCode ? ` · ${attempt.failureCode}` : ''}
                </small>
              ))}
              {externalPosts.map((post) => (
                <small key={post.id}>
                  ExternalPost {post.externalPostId}
                  {post.externalUrl ? ` · ${post.externalUrl}` : ''}
                </small>
              ))}
              {request.failureMessage && <small>{request.failureMessage}</small>}
              {nextAction === 'NEEDS_HUMAN_ACTION' && <small>NEEDS_HUMAN_ACTION · 需要人工处理后再继续</small>}
              {request.status === 'DRAFT' && (
                <p>
                  <Link className="module-nav-link" href={`/projects/${projectId}/approvals`}>
                    前往 Approval Gate
                  </Link>
                  {approval?.status === 'APPROVED' && approval.targetRevisionId === revision.id && (
                    <button type="button" disabled={busy} onClick={() => void queueRequest(request.id)}>
                      进入发布队列
                    </button>
                  )}
                </p>
              )}
              {request.status === 'PUBLISHED' && <small>PUBLISHED · 已生成 Fake Platform 发布记录</small>}
            </li>
          ))}
        </ul>
      </section>
