...
export default function ProjectLayout({ children, params }: { children: ReactNode; params: { id: string } }) {
  ...
  return <div className="project-workspace"><header className="workspace-header"><div><Link className="muted" href="/">← 返回项目中心</Link><h2>{project.name}</h2></div>{project.status && <StatusBadge status={project.status} />}</header><ProjectNav projectId={params.id} currentStage={currentStage} />{productionRunId && productionTitle && <div className="production-context"><Link href={`/projects/${params.id}/production/${productionRunId}`}>← 返回生产任务《{productionTitle}》</Link><span>当前生产任务上下文</span></div>}<div className="project-workspace-content">{children}</div></div>;
}
