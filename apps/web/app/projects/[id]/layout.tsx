...
export default function ProjectLayout({ children, params }: { children: ReactNode; params: { id: string } }) {
  ...
  useEffect(() => { if (!productionRunId) { setProductionTitle(''); return; } let active = true; void fetch(`/api/v1/projects/${params.id}/production-runs/${productionRunId}`).then((response) => response.ok ? response.json() as Promise<{ title: string }> : null).then((value) => { if (active) setProductionTitle(value?.title || ''); }); return () => { active = false; }; }, [params.id, productionRunId]);
  ...
  return <div className="project-workspace"><header className="workspace-header"><div><Link className="muted" href="/">← 返回项目中心</Link><h2>{project.name}</h2></div>{project.status && <StatusBadge status={project.status} />}</header><ProjectNav projectId={params.id} currentStage={currentStage} />{productionRunId && productionTitle && <div className="production-context"><Link href={`/projects/${params.id}/production/${productionRunId}`}>← 返回生产运行《{productionTitle}》</Link><span>当前 Production Run 上下文</span></div>}<div className="project-workspace-content">{children}</div></div>;
}
