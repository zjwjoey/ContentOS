...
const kindLabel: Record<string, string> = { VIDEO: '视频', AUDIO: '音频', VIDEO_RENDER: '成片' };
...
export default function AssetsPage({ params }: { params: { id: string } }) {
  ...
  const upload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setNotice(failed ? `${files.length - failed} 个文件已入队，${failed} 个失败。` : `${files.length} 个文件已上传并进入素材导入任务。`); await refresh(); setBusy(false); event.target.value = '';
  };

  return <main className="shell"><header><p className="eyebrow">项目 / {projectId}</p><h1>素材库</h1><p className="muted">上传只负责安全落盘和入队，校验、探测、去重与入库会在后台完成。</p><nav className="module-nav"><Link href={`/projects/${projectId}`}>项目总控</Link><Link href={`/projects/${projectId}/director`}>进入内容策划</Link><Link href={`/projects/${projectId}/avatar`}>进入数字人</Link></nav></header>
    ...
    <section className="grid"><section className="card"><div className="section-title"><h2>导入队列</h2><span>{imports.length} 条</span></div><ul className="revision-list">{imports.map((item) => <li key={item.id}><strong>{item.originalName}</strong><span>{stateLabel[item.state] || item.state} · {kindLabel[item.kind] || item.kind} · {item.byteSize} 字节</span>{item.errorMessage && <small>{item.errorMessage}</small>}</li>)}</ul>{imports.length === 0 && <p className="muted">还没有上传素材。</p>}</section><section className="card"><div className="section-title"><h2>可用素材</h2><span>{assets.length} 个</span></div><ul className="revision-list">{assets.map((asset) => <li key={asset.id}><strong>{asset.originalName}</strong><span>{kindLabel[asset.kind] || asset.kind} · {stateLabel[asset.lifecycle] || asset.lifecycle} · {asset.byteSize} 字节</span><small>{asset.metadata.width ? `${asset.metadata.width}×${asset.metadata.height}` : ''} {asset.metadata.durationMs ? `${Math.round(asset.metadata.durationMs / 1000)} 秒` : ''} {asset.metadata.tags?.length ? ` · 标签：${asset.metadata.tags.join('、')}` : ''}</small><label>标签（逗号分隔）<input defaultValue={asset.metadata.tags?.join(', ') || ''} onBlur={(event) => { const tags = event.target.value.split(/[,，]/).map((tag) => tag.trim()).filter(Boolean); void fetch(`/api/v1/projects/${projectId}/assets/${asset.id}/tags`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ tags }) }).then(() => void refresh()); }} /></label>{asset.kind === 'VIDEO' || asset.kind === 'VIDEO_RENDER' ? <video controls preload="metadata" src={`/api/v1/projects/${projectId}/assets/${asset.id}/content`} /> : <audio controls preload="metadata" src={`/api/v1/projects/${projectId}/assets/${asset.id}/content`} />}</li>)}</ul>{assets.length === 0 && <p className="muted">后台处理完成后，素材会出现在这里。</p>}</section></section>
  </main>;
}
