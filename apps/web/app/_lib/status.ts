...
const MAP: Record<string, Omit<UiStatusView, 'raw'>> = {
  ...
  REJECTED: { label: '已驳回', tone: 'danger', group: 'error' }, RECONCILING: { label: '状态确认中', tone: 'warning', group: 'attention' }, CANCELLED: { label: '已取消', tone: 'neutral', group: 'idle' }, SUPERSEDED: { label: '历史版本', tone: 'neutral', group: 'idle' }, CANCEL_REQUESTED: { label: '正在取消', tone: 'warning', group: 'attention' }, BLOCKED: { label: '已阻塞', tone: 'danger', group: 'error' }, DEDUPED: { label: '已去重', tone: 'success', group: 'ready' }, PROCESSING: { label: '处理中', tone: 'info', group: 'active' }, STAGED: { label: '准备中', tone: 'neutral', group: 'idle' },
};
