...
export function registerDashboardRoutes(app: FastifyInstance, dependencies: DashboardRouteDependencies): void {
  app.get('/api/v1/dashboard', async (_request, reply) => {
    try {
      ...
      const snapshots = await Promise.all(projects.slice(0, 20).map((project) => dependencies.center.get(project.id)));
      ...
      for (const item of items) {
        ...
        counts.pendingActions += item.actions.length;
        counts.runningJobs += item.recentJobs.filter((job) => ['QUEUED', 'RUNNING', 'RETRY_WAIT'].includes(job.state)).length;
      }
      return { counts, recentProjects: items.slice(0, 8).map((item) => ({ project: item.project, health: item.health, currentStage: item.currentStage, actions: item.actions.slice(0, 3), recentJobs: item.recentJobs.slice(0, 3) })) };
    } catch (error) {
