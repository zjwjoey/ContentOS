...
export async function buildApi(input: Pool | ApiRuntimeDependencies): Promise<FastifyInstance> {
  ...
  registerDigitalHumanRoutes(app, { digitalHuman, projects, providers: createRuntimeDigitalHumanProviders(), quickEdit, video, assets, assetService, storage, mediaStagingSecret: process.env.CONTENTOS_MEDIA_STAGING_SECRET });
