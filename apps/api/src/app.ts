...
import { DirectorVideoService, VideoProjectReadService, VideoQuickEditService, VideoService } from '../../../packages/modules/video/src/index.js';
...
export async function buildApi(input: Pool | ApiRuntimeDependencies): Promise<FastifyInstance> {
  ...
  const quickEdit = new VideoQuickEditService(db, assets);
  ...
  registerVideoRoutes(app, { projects, director: directorV1, videoFromDirector, videoRead: new VideoProjectReadService(db), assets, approvals, jobs, video, quickEdit });
