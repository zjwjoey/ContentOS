  ...
  app.post('/api/v1/projects/:projectId/video/montage-plans', async (request, reply) => {
    ...
    try {
      ...
      if (parsed.data.sourceRoot || parsed.data.scanId) {
        ...
        plannerAssets = scan.files.filter((file) => file.available).map((file) => { const id = `${scan.sourceRootId}:${file.relativePath}`; const index = indexedById.get(id); return { id, storageKey: id, sourcePath: file.sourcePath, durationMs: file.durationMs, originalName: file.fileName, tags: index?.tags || file.tags || [], usageCount: index?.usageCount || file.usageCount || 0, recentUsageCount: index?.recentUsageCount || 0, lastUsedAt: index?.lastUsedAt || file.lastUsedAt, metadata: { width: file.width, height: file.height, format: file.format, relativePath: file.relativePath, category: index?.category || file.category, usageCount: index?.usageCount || file.usageCount || 0, recentUsageCount: index?.recentUsageCount || 0, lastUsedAt: index?.lastUsedAt || file.lastUsedAt } }; });
