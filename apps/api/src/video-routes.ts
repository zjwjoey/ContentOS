...
const localMediaMetaInput = z.object({ category: z.string().max(100).nullable().optional(), tags: z.array(z.string().max(80)).max(64).optional(), gold: z.boolean().optional() });
...
const presetPatchInput = presetFields.partial().superRefine(presetDurationValidation);
const montagePlanInput = z.object({ mode: z.enum(['SCRIPT', 'RANDOM']), script: z.string().max(100_000).optional(), sentences: z.array(z.object({ index: z.number().int().nonnegative().optional(), text: z.string().trim().min(1), normalizedText: z.string().optional(), voiceStartMs: z.number().nonnegative().optional(), voiceEndMs: z.number().nonnegative().optional(), durationMs: z.number().positive().optional() })).optional(), videoAssetIds: z.array(z.string().trim().min(1)).max(256).default([]), selectedLocalMediaIds: z.array(z.string().trim().min(1)).max(256).default([]), sourceRoot: z.string().trim().min(1).optional(), scanId: z.string().trim().min(1).optional(), recursive: z.boolean().default(true), seed: z.number().int().default(1), minClipDurationMs: z.number().int().positive().default(2_000), maxClipDurationMs: z.number().int().positive().default(5_000), preferUnusedMedia: z.boolean().default(true), voiceAssetId: z.string().trim().min(1).optional(), introAssetId: z.string().trim().min(1).optional(), outroAssetId: z.string().trim().min(1).optional(), introDurationMs: z.number().int().positive().max(10_000).optional(), outroDurationMs: z.number().int().positive().max(10_000).optional() }).superRefine((value, context) => { if (!value.script?.trim() && !value.sentences?.length) context.addIssue({ code: z.ZodIssueCode.custom, path: ['script'], message: 'script or sentences is required' }); if (!value.sourceRoot && !value.scanId && value.videoAssetIds.length === 0 && value.selectedLocalMediaIds.length === 0) context.addIssue({ code: z.ZodIssueCode.custom, path: ['videoAssetIds'], message: 'videoAssetIds or local media selection is required' }); if (value.selectedLocalMediaIds.length > 0 && !value.scanId && !value.sourceRoot) context.addIssue({ code: z.ZodIssueCode.custom, path: ['scanId'], message: 'selected local media requires scanId' }); });
...
export function registerVideoRoutes(app: FastifyInstance, dependencies: VideoRouteDependencies): void {
  ...
  app.post('/api/v1/projects/:projectId/video/montage-plans', async (request, reply) => {
    ...
    try {
      const plannerAssets: Array<{ id: string; storageKey: string; sourcePath: string; durationMs: number; originalName?: string; tags?: string[]; metadata?: Record<string, unknown>; usageCount?: number; recentUsageCount?: number; lastUsedAt?: string }> = [];
      let localPoolMeta: { localMediaSourceRootId: string; localMediaScanId: string } | null = null;
      if (parsed.data.sourceRoot || parsed.data.scanId || parsed.data.selectedLocalMediaIds.length > 0) {
        if (!dependencies.localMedia) throw new Error('LOCAL_MEDIA_ROOT_UNAUTHORIZED');
        const rootId = parsed.data.scanId ? undefined : dependencies.localMedia.authorizeRoot(parsed.data.sourceRoot!).sourceRootId;
        const scan = parsed.data.scanId ? await dependencies.localMedia.getScan(parsed.data.scanId, projectId) : await dependencies.localMedia.getLatestScan(projectId, rootId!);
        ...
        const selectedIds = parsed.data.selectedLocalMediaIds.length > 0 ? new Set(parsed.data.selectedLocalMediaIds) : null;
        const chosen = scan.files.filter((file) => file.available && (!selectedIds || selectedIds.has(`${scan.sourceRootId}:${file.relativePath}`)));
      ...
      if (parsed.data.videoAssetIds.length > 0) {
        ...
        plannerAssets.push(...selected.map((asset) => ({ id: asset.id, storageKey: asset.storageKey, sourcePath: storage.objectPath(asset.storageKey), durationMs: Number(asset.metadata.durationMs || 0), ...(typeof asset.metadata.originalName === 'string' ? { originalName: asset.metadata.originalName } : asset.storageKey.split('/').at(-1) ? { originalName: asset.storageKey.split('/').at(-1)! } : {}), tags: Array.isArray(asset.metadata.tags) ? asset.metadata.tags.filter((tag): tag is string => typeof tag === 'string') : [], usageCount: Number(asset.metadata.usageCount || 0), recentUsageCount: Number(asset.metadata.recentUsageCount || 0), ...(typeof asset.metadata.lastUsedAt === 'string' ? { lastUsedAt: asset.metadata.lastUsedAt } : {}), metadata: asset.metadata })));
      }
      if (plannerAssets.length === 0) throw new Error('VIDEO_SOURCE_ASSET_INVALID');
      ...
      const plannerOptions = { projectId, sentences: effectiveSentences, assets: plannerAssets, seed: parsed.data.seed, minClipDurationMs: parsed.data.minClipDurationMs, maxClipDurationMs: parsed.data.maxClipDurationMs, preferUnusedMedia: parsed.data.preferUnusedMedia, ...(parsed.data.voiceAssetId ? { voiceAssetId: parsed.data.voiceAssetId } : {}) };
      const result = parsed.data.mode === 'SCRIPT' ? buildScriptMontageManifest({ ...plannerOptions, ...(parsed.data.script ? { script: parsed.data.script } : {}) }) : buildRandomSentenceMontageManifest(plannerOptions);
