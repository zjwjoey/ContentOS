...
export interface ProjectCenterRuleInput {
  ...
  assetImportStates?: string[];
}

function hasJob(input: ProjectCenterRuleInput, states: string[]): boolean {
  return (
    input.jobs.some((job) => states.includes(job.state)) ||
    input.videoJobStates.some((state) => states.includes(state)) ||
    states.some((state) => (input.jobStateCounts?.[state] || 0) > 0)
  );
}

function hasVideoJob(input: ProjectCenterRuleInput, states: string[]): boolean {
  return (
    input.jobs.some((job) => job.type === 'VIDEO_RENDER' && states.includes(job.state)) ||
    input.videoJobStates.some((state) => states.includes(state)) ||
    states.some((state) => (input.videoJobStateCounts?.[state] || 0) > 0)
  );
}
...
export function deriveHealth(input: ProjectCenterRuleInput): { level: ProjectCenterHealthLevel; reasons: string[] } {
  ...
  if (Object.entries(input.publisherStatusCounts).some(([status, count]) => count > 0 && publisherAttentionStatuses.has(status)))
    reasons.push('Publisher 存在待处理请求');
...
function stage(key: ProjectCenterStageKey, status: ProjectCenterStage['status'], projectId: string, summary: string): ProjectCenterStage {
  const href =
    key === 'ASSETS'
      ...
      : key === 'DIRECTOR'
        ? '/projects/' + projectId + '/director'
        : key === 'VIDEO'
          ? '/projects/' + projectId + '/video'
          : key === 'APPROVAL'
            ? '/projects/' + projectId + '/approvals'
            : '/projects/' + projectId + '/publisher';
  return { key, status, label: { ASSETS: 'Assets', DIRECTOR: 'Director', VIDEO: 'Video', APPROVAL: 'Approval', PUBLISHER: 'Publisher' }[key], href, summary };
}

export function deriveStages(input: ProjectCenterRuleInput): ProjectCenterStage[] {
  const assetImportStates = input.assetImportStates || [];
  const assetsStatus = assetImportStates.some((state) => ['FAILED', 'CANCELLED'].includes(state))
    ? 'BLOCKED'
    : assetImportStates.some((state) => ['QUEUED', 'RUNNING', 'PROCESSING'].includes(state))
      ? 'IN_PROGRESS'
      : input.projectAssetCount !== undefined
        ? input.projectAssetCount > 0
          ? 'READY'
          : 'NOT_STARTED'
        : input.hasDirectorRevision || input.hasReadyVideo
          ? 'READY'
          : 'NOT_STARTED';
  const directorStatus = input.hasApprovedDirector ? 'COMPLETE' : input.hasDirectorRevision ? 'IN_PROGRESS' : 'NOT_STARTED';
  const videoStatus = hasVideoJob(input, ['FAILED', 'BLOCKED'])
    ? 'BLOCKED'
    : hasVideoJob(input, ['QUEUED', 'RUNNING', 'RETRY_WAIT'])
      ? 'IN_PROGRESS'
      : input.hasReadyVideo
        ? 'READY'
        : 'NOT_STARTED';
  const approvalStatus =
    input.approvalStatus === 'REJECTED' || input.approvalStatuses?.includes('REJECTED')
      ? 'BLOCKED'
      : input.approvalStatus === 'PENDING' || input.approvalStatuses?.includes('PENDING')
        ? 'ACTION_REQUIRED'
        : input.approvalStatus === 'IN_PROGRESS'
          ? 'IN_PROGRESS'
  ...
  const publisherStatus =
    input.needsHumanActionCount > 0
      ? 'ACTION_REQUIRED'
      : (input.publisherStatusCounts.FAILED || 0) > 0
        ? 'BLOCKED'
        : Object.entries(input.publisherStatusCounts).some(([status, count]) => count > 0 && publisherInFlightStatuses.has(status))
          ? 'IN_PROGRESS'
          : Object.entries(input.publisherStatusCounts).some(([status, count]) => count > 0 && ['DRAFT', 'SCHEDULED'].includes(status))
            ? 'READY'
            : input.hasExternalPost
              ? 'COMPLETE'
              : hasActivePublisherRequest
                ? 'READY'
                : 'NOT_STARTED';
  return [
    stage(
      ...
      input.projectId,
      assetsStatus === 'READY'
        ? `${input.projectAssetCount || 0} 个项目素材已就绪`
        : assetsStatus === 'IN_PROGRESS'
          ? '素材正在导入'
          : assetsStatus === 'BLOCKED'
            ? '素材导入失败，需要处理'
            : '尚未导入项目素材',
    ),
    stage(
      ...
      directorStatus,
      input.projectId,
      directorStatus === 'COMPLETE' ? '已批准 Director 版本' : directorStatus === 'IN_PROGRESS' ? 'Director 仍在准备' : '尚未创建 Director 版本',
    ),
    stage(
      'VIDEO',
      videoStatus,
    ...
    stage(
      ...
      publisherStatus === 'COMPLETE'
        ? '已确认外部发布'
        : publisherStatus === 'ACTION_REQUIRED'
          ? '需要人工处理发布账号'
...
export interface ProjectCenterServiceDependencies {
  ...
  assetImports?: Pick<AssetImportService, 'list'>;
  jobs: JobService;
...
export function currentApprovalRecords(
  approvals: Array<{ targetType: string; targetId: string; targetRevisionId: string; status: string; revision: number; createdAt: string }>,
  currentPublisherRevisionKeys: Set<string>,
  _currentRenderRevisionId: string | null,
  currentDirectorTargets: CurrentDirectorApprovalTargets = {},
  currentRenderTarget?: { targetId: string; targetRevisionId: string },
): CurrentApprovalSummary[] {
  const current = new Map<string, { revision: number; createdAt: string; summary: CurrentApprovalSummary }>();
  for (const approval of approvals) {
    if (approval.targetType !== 'RENDER' && approval.targetType !== 'PUBLISH') continue;
    ...
    if (
      approval.targetType === 'RENDER' &&
      currentRenderTarget &&
      key !== 'RENDER:' + currentRenderTarget.targetId + ':' + currentRenderTarget.targetRevisionId
    )
      continue;
    if (approval.targetType === 'RENDER' && !currentRenderTarget) continue;
    const previous = current.get(key);
    if (!previous || approval.revision > previous.revision || (approval.revision === previous.revision && approval.createdAt > previous.createdAt))
      current.set(key, {
        revision: approval.revision,
        createdAt: approval.createdAt,
        summary: { targetType: approval.targetType, targetId: approval.targetId, targetRevisionId: approval.targetRevisionId, status: approval.status },
      });
  }
...
export function buildActions(
  ...
  approvalStatus: string | null,
  summary: PublisherProjectSummary,
  jobs: JobSummary[],
  failedSources: string[] = [],
  jobStateCounts: Record<string, number> = {},
  historicalJobs: JobSummary[] = [],
  approvalStatuses: string[] = [],
  approvalTargets: Array<{ targetType: string; status: string }> = [],
): ProjectCenterAction[] {
  const actions: ProjectCenterAction[] = [];
  for (const source of failedSources)
    actions.push({
      id: 'source-unavailable-' + source,
      kind: 'NAVIGATION',
      title: source + ' 数据暂时不可用',
      detail: '请刷新项目总控或进入项目页面继续检查。',
      severity: 'BLOCKED',
      href: '/projects/' + projectId,
    });
  const approvalActionHref = (targetType: string): string | null =>
    targetType === 'PUBLISH' ? '/projects/' + projectId + '/publisher' : targetType === 'RENDER' ? '/projects/' + projectId + '/video' : null;
  const addApprovalAction = (status: string, targetType: string): void => {
    const suffix = targetType === 'PUBLISH' ? '' : '-' + targetType;
    if (status === 'PENDING')
      actions.push({
        id: 'approval-pending' + suffix,
        kind: 'APPROVAL',
        title: '处理待审批内容',
        detail: targetType + ' 当前版本等待人工审批。',
        severity: 'WARNING',
        href: approvalActionHref(targetType),
      });
    if (status === 'REJECTED')
      actions.push({
        id: 'approval-rejected' + suffix,
        kind: 'APPROVAL',
        title: '处理被驳回审批',
        detail: targetType + ' 当前版本审批未通过，请检查并更新内容。',
        severity: 'BLOCKED',
        href: approvalActionHref(targetType),
      });
    if (status === 'MISSING')
      actions.push({
        id: 'approval-missing' + suffix,
        kind: 'APPROVAL',
        title: '补充当前版本审批',
        detail: targetType + ' 当前版本尚未形成审批决定。',
        severity: 'WARNING',
        href: approvalActionHref(targetType),
      });
  };
  if (approvalTargets.length > 0) {
    for (const targetType of [...new Set(approvalTargets.map((target) => target.targetType))]) {
      for (const status of [...new Set(approvalTargets.filter((target) => target.targetType === targetType).map((target) => target.status))])
        addApprovalAction(status, targetType);
    }
  } else {
  ...
  if (summary.needsHumanActionCount > 0)
    actions.push({
      id: 'publisher-human-action',
      kind: 'HUMAN_ACTION',
      title: '处理发布账号',
      detail: 'Publisher 有需要人工处理的账号或外部状态。',
      severity: 'BLOCKED',
      href: '/projects/' + projectId + '/publisher',
    });
  ...
  for (const job of [...jobs, ...historicalJobs].filter((item) => ['FAILED', 'BLOCKED'].includes(item.state))) {
    ...
    actions.push({
      ...
      kind: 'JOB_FAILURE',
      title: '处理失败 Job',
      detail: job.id + ' · ' + job.type + ' 已进入 ' + job.state + '。',
      severity: 'BLOCKED',
      href: job.type === 'PUBLISH' ? '/projects/' + projectId + '/publisher' : null,
    });
  }
  ...
  if (unresolvedFailureCount > recentFailureCount)
    actions.push({
      id: 'job-failure-aggregate',
      kind: 'JOB_FAILURE',
      title: '还有历史失败 Job',
      detail: '还有 ' + (unresolvedFailureCount - recentFailureCount) + ' 个较早的失败或阻塞 Job 未显示在最近列表中，请继续检查 Job 状态。',
      severity: 'BLOCKED',
      href: null,
    });
  if ((summary.statusCounts.FAILED || 0) > 0 && summary.needsHumanActionCount === 0)
    actions.push({
      id: 'publisher-retry',
      kind: 'PUBLISH_RETRY',
      title: '检查发布失败请求',
      detail: 'Publisher 存在失败请求，可进入工作台处理。',
      severity: 'WARNING',
      href: '/projects/' + projectId + '/publisher',
    });
  if (actions.length === 0 && health.level === 'HEALTHY')
    actions.push({
      id: 'open-director',
      kind: 'NAVIGATION',
      title: '进入 Director',
      detail: '继续完善项目内容规划。',
      severity: 'INFO',
      href: '/projects/' + projectId + '/director',
    });
  return actions;
}
...
export class ProjectCenterService {
  ...
  async get(projectId: string): Promise<ProjectCenterSnapshot | null> {
    ...
    const [director, assets, assetImports, currentRender, jobs, failedJobs, jobStates, approvals, publisher, publisherRequests] = await Promise.all([
      read(() => this.dependencies.director.get(projectId)),
      read<AssetSummaryV0[]>(async () =>
        this.dependencies.assets.listProjectAssets
          ? this.dependencies.assets.listProjectAssets(projectId)
          : ((await this.dependencies.assets.listPublishable(projectId)) as unknown as AssetSummaryV0[]),
      ),
      read(() => this.dependencies.assetImports?.list(projectId) || Promise.resolve([])),
    ...
    const currentPublisherTargets: CurrentApprovalTarget[] = (publisherRequests.value || [])
      .filter((request) => request.status !== 'CANCELLED' && request.currentRevisionId)
      .map((request) => ({
        key: 'PUBLISH:' + request.id + ':' + request.currentRevisionId,
        targetType: 'PUBLISH',
        targetId: request.id,
        targetRevisionId: request.currentRevisionId!,
      }));
    const currentPublisherRevisionKeys = new Set(currentPublisherTargets.map((target) => target.key));
    const currentDirectorTargets =
      directorSummary?.source === 'V1'
        ? {
            ...(directorSummary.activeScript
              ? { script: { targetId: directorSummary.activeScript.aggregateId, targetRevisionId: directorSummary.activeScript.revisionId } }
              : {}),
            ...(directorSummary.activeStoryboard
              ? { storyboard: { targetId: directorSummary.activeStoryboard.aggregateId, targetRevisionId: directorSummary.activeStoryboard.revisionId } }
              : {}),
          }
        : {};
    const currentRenderTarget =
      currentRender.value && (assets.value || []).some((asset) => asset.id === currentRender.value?.outputAssetId && asset.kind === 'VIDEO_RENDER')
        ? currentRender.value
        : undefined;
    const currentRenderApprovalTarget = currentRenderTarget
      ? { targetId: currentRenderTarget.renderId, targetRevisionId: currentRenderTarget.outputAssetId }
      : undefined;
    ...
    const currentApprovalTargets: CurrentApprovalTarget[] = [
      ...
      ...(currentRenderApprovalTarget
        ? [
            {
              key: 'RENDER:' + currentRenderApprovalTarget.targetId + ':' + currentRenderApprovalTarget.targetRevisionId,
              targetType: 'RENDER',
              targetId: currentRenderApprovalTarget.targetId,
              targetRevisionId: currentRenderApprovalTarget.targetRevisionId,
            },
          ]
        : []),
    ];
    ...
    const ruleInput: ProjectCenterRuleInput = {
      ...
      approvalStatuses,
      projectAssetCount: assets.value?.length || 0,
      assetImportStates: assetImports.value?.map((item) => item.state) || [],
    };
    ...
    const failedSources = Array.from(
      new Set(
        [
          ...
          assets.failed || assetImports.failed ? 'Assets' : null,
    ...
    return {
      ...
      actions: buildActions(
        ...
        jobSummaries,
        failedSources,
        jobStateSummary.stateCounts,
        failedJobs.value || [],
        approvalStatuses,
        currentApprovalTargetStatuses,
      ),
      recentJobs: jobSummaries.map((job) => ({
        id: job.id,
        type: job.type,
        state: job.state,
