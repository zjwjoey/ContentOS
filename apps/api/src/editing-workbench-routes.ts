...
export function registerEditingWorkbenchRoutes(app: FastifyInstance, dependencies: EditingWorkbenchRouteDependencies): void {
  ...
  app.get('/api/v1/edit/history', async (request, reply) => {
    ...
    return { page: parsed.data.page, pageSize: parsed.data.pageSize, items: rows.rows.map((row) => { const settings = row.settings && typeof row.settings === 'object' && !Array.isArray(row.settings) ? row.settings as Record<string, unknown> : {}; const presentation = settings.presentationSettings && typeof settings.presentationSettings === 'object' ? settings.presentationSettings as { canvas?: { aspectRatio?: string; width?: number; height?: number; fitMode?: string }; subtitleStyle?: { animation?: string } } : undefined; return { id: String(row.id), title: String(row.title), mode: String(row.mode), status: String(row.status), totalCount: Number(row.total_count), succeededCount: Number(row.succeeded_count), failedCount: Number(row.failed_count), createdAt: new Date(String(row.created_at)).toISOString(), outputRoot: row.output_root || null, testOnly: settings.testOnly === true, ...(presentation?.canvas ? { presentation: { aspectRatio: presentation.canvas.aspectRatio, width: presentation.canvas.width, height: presentation.canvas.height, fitMode: presentation.canvas.fitMode, animation: presentation.subtitleStyle?.animation } } : {}) }; }) };
  });
