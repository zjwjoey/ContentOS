...
const presentationSettingsSchema = z.object({ schemaVersion: z.literal('EDIT_PRESENTATION_V1').default('EDIT_PRESENTATION_V1'), canvas: z.object({ aspectRatio: z.enum(['9:16', '16:9', '1:1']), width: z.number().int().positive(), height: z.number().int().positive(), fps: z.number().int().min(1).max(120), fitMode: z.enum(['FILL', 'CONTAIN', 'BLUR_BACKGROUND']) }), subtitleStyle: z.record(z.string(), z.unknown()), segmentation: z.object({ mode: z.enum(['COMMA_SENTENCE', 'SENTENCE_ONLY', 'CUSTOM']), delimiters: z.array(z.string()).optional() }) });
