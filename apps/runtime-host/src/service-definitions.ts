...
export function createServiceDefinitions(options: RuntimeServicesOptions): ServiceDefinition[] {
  ...
  const api: ServiceDefinition = { ...processService('api', 'API', 'apps/api/src/main.ts', options, ['database', 'migration'], true), port: Number(env.PORT || 3000), healthCheck: async () => { const response = await fetch(`http://127.0.0.1:${Number(env.PORT || 3000)}/health`); return response.ok ? { state: 'READY', message: 'API liveness ready' } : { state: 'FAILED', message: `HTTP ${response.status}` }; } };
