...
export interface RuntimeServicesOptions { appRoot: string; env: Record<string, string | undefined>; safeMode: boolean; }
const tsx = (appRoot: string) => resolve(appRoot, 'node_modules', 'tsx', 'dist', 'cli.mjs');
const processService = (id: string, label: string, entry: string, options: RuntimeServicesOptions, dependsOn: string[], required: boolean): ServiceDefinition => ({ id, label, kind: 'PROCESS', required, dependsOn, startupTimeoutMs: 45_000, shutdownTimeoutMs: 10_000, restartClass: 'TRANSIENT', restartPolicy: { enabled: true, maxRestarts: 3, windowMs: 60_000, backoffMs: [1_000, 3_000, 10_000] }, command: process.execPath, args: [tsx(options.appRoot), resolve(options.appRoot, entry)], env: { NODE_ENV: 'development' } });

export function createServiceDefinitions(options: RuntimeServicesOptions): ServiceDefinition[] {
  const env = options.env; const databaseUrl = env.DATABASE_URL || 'postgresql://contentos_dev:change-me@127.0.0.1:55433/contentos_operator_dev';
  ...
  const api: ServiceDefinition = { ...processService('api', 'API', 'apps/api/src/main.ts', options, ['database', 'migration'], true), port: Number(env.PORT || 3000), healthCheck: async () => { const response = await fetch(`http://127.0.0.1:${Number(env.PORT || 3000)}/ready`); return response.ok ? { state: 'READY', message: 'API readiness ready' } : { state: 'FAILED', message: `HTTP ${response.status}` }; } };
  const web: ServiceDefinition = { id: 'web', label: 'Web', kind: 'PROCESS', required: true, dependsOn: ['api'], startupTimeoutMs: 60_000, shutdownTimeoutMs: 10_000, restartClass: 'TRANSIENT', restartPolicy: { enabled: true, maxRestarts: 3, windowMs: 60_000, backoffMs: [1_000, 3_000, 10_000] }, command: process.execPath, args: [resolve(options.appRoot, 'apps/web/node_modules/next/dist/bin/next'), 'dev', '-p', String(env.WEB_PORT || 3001), resolve(options.appRoot, 'apps/web')], env: { NODE_ENV: 'development', CONTENTOS_API_URL: `http://127.0.0.1:${Number(env.PORT || 3000)}` }, port: Number(env.WEB_PORT || 3001), healthCheck: async () => { const response = await fetch(`http://127.0.0.1:${Number(env.WEB_PORT || 3001)}/`); return response.ok ? { state: 'READY', message: 'Web reachable' } : { state: 'FAILED', message: `HTTP ${response.status}` }; } };
