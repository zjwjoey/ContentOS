...
function invalid(message: string): Error {
  return Object.assign(new Error(message), { code: 'REVIEW_PAYLOAD_INVALID', retryable: false });
}
...
export function createReviewJobHandler(deps: ReviewWorkerDependencies): (job: JobRecord, attemptId: string, signal: AbortSignal) => Promise<unknown> {
  return async (job, attemptId, signal) => {
    ...
    if (job.type === REVIEW_GENERATE_ANALYSIS) {
      if (
        !deps.ai ||
        payload.schemaVersion !== 'REVIEW_ANALYSIS_JOB_V1' ||
        payload.projectId !== job.projectId ||
        typeof payload.externalPostId !== 'string' ||
        !Array.isArray(payload.metricSnapshotIds) ||
        payload.metricSnapshotIds.length === 0
      )
        throw invalid('Invalid Review analysis Job payload');
      ...
      if (snapshots.length !== snapshotIds.length)
        throw Object.assign(new Error('Metric snapshots not found for project/post'), { code: 'REVIEW_SNAPSHOT_NOT_FOUND', retryable: false });
      ...
      const aiResult = await deps.ai.generateStructured(
        {
          ...
          variables: {
            ...
            publishedAt: latest.publishedAt || latest.capturedAt,
            metrics: JSON.stringify(latest.metrics),
            history: JSON.stringify(snapshots.map((snapshot) => ({ capturedAt: snapshot.capturedAt, metrics: snapshot.metrics }))),
          },
        },
        (value) => {
          ...
          validateReviewAnalysisReportV1({
            ...
            highlights: candidate.highlights as never,
            risks: candidate.risks as never,
            recommendations: candidate.recommendations as never,
            aiRunId: 'validation',
      ...
      const result = await deps.jobs.succeedWithCurrentAttempt(job.id, attemptId, async () => {
        const report = await deps.analytics.recordAnalysisReport({
          schemaVersion: 'REVIEW_ANALYSIS_REPORT_V1',
          id: `review-report-${randomUUID()}`,
          projectId: String(payload.projectId),
          externalPostId: String(payload.externalPostId),
          metricSnapshotIds: snapshotIds,
          summary: String((aiResult.output as Record<string, unknown>).summary),
          highlights: (aiResult.output as Record<string, unknown>).highlights as never,
          risks: (aiResult.output as Record<string, unknown>).risks as never,
          recommendations: (aiResult.output as Record<string, unknown>).recommendations as never,
    ...
    if (
      job.type !== REVIEW_COLLECT_METRICS ||
      payload.schemaVersion !== 'REVIEW_METRIC_COLLECTION_JOB_V1' ||
      payload.projectId !== job.projectId ||
      typeof payload.externalPostId !== 'string' ||
      !['FAKE', 'IMPORT'].includes(String(payload.source))
    )
      throw invalid('Invalid Review metric collection Job payload');
