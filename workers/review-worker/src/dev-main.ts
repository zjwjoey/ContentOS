...
async function startLocalWorker(): Promise<void> {
  ...
  const worker = createReviewWorker({ jobs, analytics, posts: publisher, metricsSource: new FakeMetricsSource(), workerId: 'review-worker-dev' });
  await worker.start();
  const poll = async () => {
    const runnable = await jobs.listRunnable([REVIEW_COLLECT_METRICS], 10);
