...
export async function startLocalWorker(): Promise<void> {
  const config = loadConfig(); const db = await createDatabase(config.databaseUrl); await migrateUp(db); const storage = new LocalStorageProvider(config.storageRoot); const jobs = new JobService(db); const assets = new AssetCatalogService(db); const providers = createRuntimeDigitalHumanProviders({ CONTENTOS_SPEECH_PROVIDER: config.digitalHumanSpeechProvider, CONTENTOS_INDEXTTS_BASE_URL: config.indexttsBaseUrl, CONTENTOS_AVATAR_PROVIDER: config.digitalHumanAvatarProvider, CONTENTOS_MEDIA_STAGING_PROVIDER: config.mediaStagingProvider, CONTENTOS_MEDIA_STAGING_BASE_URL: config.mediaStagingBaseUrl, CONTENTOS_MEDIA_STAGING_SECRET: config.mediaStagingSecret, CONTENTOS_PROVIDER_REQUEST_TIMEOUT_MS: String(config.digitalHumanProviderRequestTimeoutMs), CONTENTOS_PROVIDER_CAPABILITY_TIMEOUT_MS: String(config.digitalHumanProviderCapabilityTimeoutMs) });
  const dependencies: DigitalHumanWorkerDependencies = { jobs, digitalHuman: new DigitalHumanService(db, jobs, assets), assets, assetService: new AssetService(db, storage, (path) => probeMedia(path, config.ffprobePath)), storage, speechProvider: providers.speech, avatarProvider: providers.avatar, staging: providers.staging, maxRemoteResultBytes: config.assetUploadMaxBytes, remoteResultTimeoutMs: config.digitalHumanRemoteResultTimeoutMs };
